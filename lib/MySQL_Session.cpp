...
void Query_Info::query_parser_init() {
  // QueryParserArgs=GloQPro->query_parser_init((char
  // *)QueryPointer,QueryLength,0);
...
unsigned long long Query_Info::query_parser_update_counters() {
  ...
  if (MyComQueryCmd == MYSQL_COM_QUERY_UNKNOWN)
    return 0; // this means that it was never initialized
              //	} else {
              //		if
              //(stmt_info->MyComQueryCmd==MYSQL_COM_QUERY___NONE) return 0; //
  // this means that it was never initialized 		if
  //(stmt_info->MyComQueryCmd==MYSQL_COM_QUERY_UNKNOWN) return 0; // this means
...
bool Query_Info::is_select_NOT_for_update() {
  // to avoid an expensive strlen() on the digest_text, we consider only the
  // real query
...
// scan the pointer array of mysql backends (mybes) looking for a backend for
// the specified hostgroup_id
...
void MySQL_Session::writeout() {
  ...
  if (mybe && mybe->server_myds &&
    ...
    if (admin == false) {
      if (mybe->server_myds->net_failure == false) {
        ...
        // (mybe->server_myds->mypolls->fds[mybe->server_myds->poll_fds_idx].revents
        // & POLLOUT)) {
        if (mybe->server_myds->poll_fds_idx >
            -1) { // NOTE: attempt to force writes
  ...
  if (client_myds) {
    if (mirror == false) {
      ...
      if (retbytes ==
          QUEUE_T_DEFAULT_SIZE) { // optimization to solve memory bloat
        runloop = true;
      }
      while (runloop) {
  ...
  // if (server_myds && server_myds->net_failure==false)
  // server_myds->write_to_net_poll();
  if (mybe) {
    if (mybe->server_myds)
        ...
        if (fd == 0) {
          autocommit = false; // we set it, no matter if already set or not
                              /*
                                                                      if (nTrx) {
                                                                              // there is an active
                                 transaction, we need to forward it
                                                                              // because this can
                                 potentially close the transaction
                                                                              autocommit_on_hostgroup=FindOneActiveTransaction();
                                                                              return false;
                                                                      } else {
                                                                              // as there is no
                                 active transaction, we do no need to forward it
                                                                              // just return OK
                                                                              goto
                                 __ret_autocommit_OK;
                                                                      }
...
bool MySQL_Session::handler_special_queries(PtrSize_t *pkt) {
  ...
  if (pkt->size == SELECT_LAST_INSERT_ID_LEN + 5 &&
    ...
    sid++;
    myprot->generate_pkt_field(true, NULL, NULL, sid, (char *)"", (char *)"",
                               (char *)"", (char *)"LAST_INSERT_ID()",
                               (char *)"", 33, 15, MYSQL_TYPE_VAR_STRING, 1,
                               0x1f, false, 0, NULL);
    sid++;
  ...
  if (pkt->size == strlen((char *)"select USER()") + 5 &&
    ...
    char *query2 = (char *)malloc(
        strlen(query1) + strlen(client_myds->myconn->userinfo->username) + 10);
    sprintf(query2, query1, client_myds->myconn->userinfo->username);
    char *error;
    int cols;
    int affected_rows;
    SQLite3_result *resultset;
    GloAdmin->admindb->execute_statement(query2, &error, &cols, &affected_rows,
                                         &resultset);
...
void MySQL_Session::
  ...
  if (pktH->size < 15 * 1024 * 1024 &&
    ...
    newsess->client_myds->myprot.init(
        &newsess->client_myds, newsess->client_myds->myconn->userinfo, newsess);
    newsess->to_process = 1;
    newsess->default_hostgroup = default_hostgroup;
    if (qpo->mirror_hostgroup >= 0) {
      newsess->mirror_hostgroup = qpo->mirror_hostgroup; // in the new session
                                                         // we copy the mirror
                                                         // hostgroup
    } else {
...
int MySQL_Session::handler_again___status_PINGING_SERVER() {
  ...
  //					thread->mypolls.add(POLLIN|POLLOUT, myds->fd,
  //myds,  thread->curtime);
  //				}
  if (rc == 0) {
    myconn->async_state_machine = ASYNC_IDLE;
    // if ((myconn->reusable==true) && ((myds->myprot.prot_status &
    ...
    if (rc == -1 || rc == -2) {
      if (rc == -2) {
        ...
        proxy_error("Detected a broken connection during ping on (%d,%s,%d) , "
  ...
  if (myds->myconn && myds->myconn->mysql) {
    if (myds->killed_at == 0) {
      ...
      KillArgs *ka = new KillArgs(
          ui->username, auth_password, myds->myconn->parent->address,
          myds->myconn->parent->port, myds->myconn->mysql->thread_id);
      pthread_attr_t attr;
...
bool MySQL_Session::handler_again___verify_backend_charset() {
  if (client_myds->myconn->options.charset !=
    ...
    switch (status) { // this switch can be replaced with a simple
                      // previous_status.push(status), but it is here for
                      // readibility
  ...
  if (rc == 0) {
    ...
    if (rc == -1) {
      ...
      if (myerr > 2000) {
        ...
        proxy_error("Detected a broken connection while setting INIT CONNECT "
                    "on %s , %d : %d, %s\n",
                    myconn->parent->address, myconn->parent->port, myerr,
                    mysql_error(myconn->mysql));
        // if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status &
        // SERVER_STATUS_IN_TRANS)==0)) {
...
bool MySQL_Session::handler_again___status_CONNECTING_SERVER(int *_rc) {
  ...
  if (mybe->server_myds->myconn == NULL) {
    ...
    *_rc = 1;
    ...
    // mybe->server_myds->myprot.init(&mybe->server_myds,
    // mybe->server_myds->myconn->userinfo, this);
    /* */
    ...
    case 0:
      ...
      if (session_fast_forward == true) {
        ...
        // set DSS=STATE_SLEEP or it will believe it have to use MARIADB client
        // library
        myds->DSS = STATE_SLEEP;
      }
      NEXT_IMMEDIATE_NEW(st);
    ...
    case -2:
      ...
      if (myds->connect_retries_on_failure > 0) {
        ...
        int myerr = mysql_errno(myconn->mysql);
        switch (myerr) {
        case 1226: // ER_USER_LIMIT_REACHED , User '%s' has exceeded the '%s'
  ...
  if (rc == 0) {
    ...
    if (rc == -1) {
      ...
      if (myerr > 2000) {
        bool retry_conn = false;
        // client error, serious
        proxy_error("Detected a broken connection during change user on %s, %d "
                    ": %d, %s\n",
                    myconn->parent->address, myconn->parent->port, myerr,
                    mysql_error(myconn->mysql));
        // if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status &
  ...
  if (rc == 0) {
    ...
    if (rc == -1) {
      ...
      if (myerr > 2000) {
        bool retry_conn = false;
        // client error, serious
        proxy_error("Detected a broken connection during SET NAMES on %s , %d "
                    ": %d, %s\n",
                    myconn->parent->address, myconn->parent->port, myerr,
                    mysql_error(myconn->mysql));
        // if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status &
  ...
  if (rc == 0) {
    ...
    if (rc == -1) {
      ...
      if (myerr > 2000) {
        bool retry_conn = false;
        // client error, serious
        proxy_error("Detected a broken connection during SET AUTOCOMMIT on %s "
                    ", %d : %d, %s\n",
                    myconn->parent->address, myconn->parent->port, myerr,
                    mysql_error(myconn->mysql));
        // if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status &
    ...
    case WAITING_CLIENT_DATA:
      ...
      case STATE_SLEEP_MULTI_PACKET:
        if (client_myds->multi_pkt.ptr == NULL) {
          ...
          memcpy((char *)client_myds->multi_pkt.ptr + tmp_pkt.size,
        ...
        if (pkt.size ==
            (0xFFFFFF + sizeof(mysql_hdr))) { // there are more packets
      ...
      case STATE_SLEEP: // only this section can be executed ALSO by mirror
        ...
        if (session_fast_forward == true) { // if it is fast forward
          ...
          mybe->server_myds->reinit_queues(); // reinitialize the queues in the
                                              // myds . By default, they are not
                                              // active
          mybe->server_myds->PSarrayOUT->add(pkt.ptr,
                                             pkt.size); // move the first packet
          previous_status.push(FAST_FORWARD);           // next status will be
                                              // FAST_FORWARD . Now we need a
                                              // connection
          NEXT_IMMEDIATE(CONNECTING_SERVER); // we create a connection . next
        ...
        case _MYSQL_COM_QUERY:
          ...
          if (admin == false) {
            ...
            if (session_fast_forward == false) {
              ...
              // shortly after, the packets it used to contain the query will be
              // deallocated
            ...
            assert(qpo); // GloQPro->process_mysql_query() should always return
                         // a qpo
            rc_break =
            ...
            //	// GloQPro->process_mysql_query() knows if we are inside a
            // mirror session
            ...
            // if a number of retries is set in mysql_query_rules, that takes
            // priority
            if (qpo) {
              if (qpo->retries >= 0) {
            ...
            if (mysql_thread___default_query_delay) {
              pause_until =
                  thread->curtime + mysql_thread___default_query_delay * 1000;
            }
            if (qpo) {
              if (qpo->delay > 0) {
                if (pause_until == 0)
            ...
            proxy_debug(
                PROXY_DEBUG_MYSQL_COM, 5,
                "Received query to be processed with MariaDB Client library\n");
        ...
        case _MYSQL_COM_STMT_CLOSE: {
          ...
          //							client_myds->setDSS_STATE_QUERY_SENT_NET();
          //							client_myds->myprot.generate_pkt_ERR(true,NULL,NULL,1,1045,(char
          //*)"#28000",(char *)"Command not supported");
        ...
        case _MYSQL_COM_STMT_PREPARE:
          if (admin ==
              true) { // admin module will not support prepared statement!!
            l_free(pkt.size, pkt.ptr);
            client_myds->setDSS_STATE_QUERY_SENT_NET();
            client_myds->myprot.generate_pkt_ERR(
            ...
            // Note: CurrentQuery sees the query as sent by the client.
            // shortly after, the packets it used to contain the query will be
            // deallocated Note2 : we call the next function as if it was
            // _MYSQL_COM_QUERY because the offset will be identical
            CurrentQuery.begin((unsigned char *)pkt.ptr, pkt.size, true);
            ...
            //(Session_STMT_Manager==NULL)
            //{
            //										Session_STMT_Manager =
            //new  MySQL_STMT_Manager();
            //									}
            ...
            //									if
            //(mysql_thread___stmt_multiplexing)
            //{
            stmt_info = GloMyStmt->find_prepared_statement_by_hash(
                hash); // find_prepared_statement_by_hash() always increase
                       // ref_count_client
                       //									}
                       //else
                       //{
            ...
            if (stmt_info) {
              ...
              // FIXME: it is possible that multiple statement have the same
              // hash
              // FIXME: we should check local_stmts to verify is this stmt_id
              // was already sent
              if (client_myds->myconn->local_stmts->exists(
                      stmt_info->statement_id)) {
                // the client is asking to prepare another identical prepared
                // statements
                __sync_fetch_and_sub(&stmt_info->ref_count_client,
                                     1); // since
                                         // find_prepared_statement_by_hash()
                                         // already increased red_count_client
                                         // we decrease it here
                stmt_info = NULL;
              }
            }
            if (stmt_info) {
              ...
              client_myds->myprot.generate_STMT_PREPARE_RESPONSE(
                  client_myds->pkt_sid + 1, stmt_info);
              //__sync_fetch_and_sub(&stmt_info->ref_count,1); // decrease
              // reference count
              client_myds->myconn->local_stmts->insert(stmt_info->statement_id,
                                                       NULL);
              __sync_fetch_and_sub(&stmt_info->ref_count_client,
                                   1); // since
                                       // find_prepared_statement_by_hash()
                                       // already increased red_count_client
                                       // before insert(), we decrease it here
              client_myds->DSS = STATE_SLEEP;
              status = WAITING_CLIENT_DATA;
              ...
              pause_until = 0;
              mybe->server_myds->killed_at = 0;
              mybe->server_myds->mysql_real_query.init(&pkt); // fix memory leak
                                                              // for PREPARE in
                                                              // prepared
                                                              // statements #796
              client_myds->setDSS_STATE_QUERY_SENT_NET();
        ...
        case _MYSQL_COM_STMT_EXECUTE:
          if (admin ==
            ...
            //{
            stmt_info =
                GloMyStmt->find_prepared_statement_by_stmt_id(stmt_global_id);
            //									}
            //else
            //{
            ...
            // *stmt_meta=client_myds->myprot.get_binds_from_pkt(pkt.ptr,pkt.size,stmt_info->num_params);

            // we now take the metadata associated with STMT_EXECUTE from
            // MySQL_STMTs_meta
            ...
            if (stmt_meta_found == false) {
              ...
              // but as we reached here, stmt_meta is not null and we save the
              // metadata
              sess_STMTs_meta->insert(stmt_global_id, stmt_meta);
            }
            // else

            //									CurrentQuery.begin((unsigned
            //char
            //*)stmt_info->query, stmt_info->query_length,false);
            CurrentQuery.stmt_meta = stmt_meta;
            //									qpo=GloQPro->process_mysql_query(this,pkt.ptr,pkt.size,&CurrentQuery);
            //// FIXME: not sure we need to call this
            //									assert(qpo);	//
            //GloQPro->process_mysql_query()  should always return a qpo
      ...
      default:
                    ...
                    "Statuses: WAITING_CLIENT_DATA - STATE_UNKNOWN\n");
        {
          ...
          case AF_INET: {
            ...
            inet_ntop(client_myds->client_addr->sa_family, &ipv4->sin_addr, buf,
                      INET_ADDRSTRLEN);
            break;
          }
          case AF_INET6: {
          ...
          proxy_error("Unexpected packet from client %s . Session_status: %d , "
...
handler_again:
  ...
  case PROCESSING_QUERY:
    ...
    if (mysql_thread___connect_timeout_server_max) {
      if (mybe->server_myds->max_connect_time == 0)
        mybe->server_myds->max_connect_time =
            thread->curtime + mysql_thread___connect_timeout_server_max * 1000;
    } else {
      mybe->server_myds->max_connect_time = 0;
    }
    if ((mybe->server_myds->myconn &&
    ...
    if (mybe->server_myds->DSS == STATE_NOT_INITIALIZED) {
      ...
      MySQL_Data_Stream *myds = mybe->server_myds;
      MySQL_Connection *myconn = myds->myconn;
      // these checks need to be performed only if we connect to a real
      // mysql_server
      // status=PROCESSING_QUERY;
      mybe->server_myds->max_connect_time = 0;
      ...
      if (default_hostgroup >= 0) {
        ...
        if (mirror ==
            false) { // do not care about autocommit and charset if mirror
          ...
          if (status == PROCESSING_STMT_EXECUTE) {
            CurrentQuery.mysql_stmt =
                myconn->local_stmts->find(CurrentQuery.stmt_global_id);
            if (CurrentQuery.mysql_stmt == NULL) {
              MySQL_STMT_Global_info *stmt_info = NULL;
              // the conection we too doesn't have the prepared statements
              // prepared we try to create it now
      ...
      // FIXME : this part of the code is commented just for reference. It seems
      // it has been removed in v1.2, as copied above
      //				//status=PROCESSING_QUERY;
      //				mybe->server_myds->max_connect_time=0;
      //				// we insert it in mypolls only if not
      // already there 				if (myds->mypolls==NULL) {
      //					thread->mypolls.add(POLLIN|POLLOUT,
      // mybe->server_myds->fd, mybe->server_myds, thread->curtime);
      //				}
      ...
      switch (status) {
      case PROCESSING_QUERY:
        rc = myconn->async_query(myds->revents, myds->mysql_real_query.QueryPtr,
                                 myds->mysql_real_query.QuerySize);
        break;
      case PROCESSING_STMT_PREPARE:
        // rc=myconn->async_query(myds->revents,
        // myds->mysql_real_query.QueryPtr,myds->mysql_real_query.QuerySize,&CurrentQuery.mysql_stmt);
          ...
          */
          /*
             ...
             (CurrentQuery.mysql_stmt==NULL) {
                                                                          // the
             conection we too doesn't have the prepared statements prepared
                                                                          // we
             try to create it now MySQL_STMT_Global_info *stmt_info=NULL;
                                                                          stmt_info=GloMyStmt->find_prepared_statement_by_stmt_id(CurrentQuery.stmt_global_id);
                                                                          CurrentQuery.QueryLength=stmt_info->query_length;
                                                                          CurrentQuery.QueryPointer=(unsigned
             char *)stmt_info->query;
                                                                          previous_status.push(PROCESSING_STMT_EXECUTE);
                                                                          NEXT_IMMEDIATE(PROCESSING_STMT_PREPARE);
                                                                  }
          */
      ...
      thread->status_variables.backend_query_time =
          ...
          (begint.tv_sec * 1000000000 + begint.tv_nsec);
      //				if
      //(myconn->async_state_machine==ASYNC_QUERY_END) {
      if (rc == 0) {
        ...
        case PROCESSING_STMT_PREPARE: {
          ...
          stmt_info = GloMyStmt->add_prepared_statement(
              &is_new, current_hostgroup,
              (char *)client_myds->myconn->userinfo->username,
              (char *)client_myds->myconn->userinfo->schemaname,
              (char *)CurrentQuery.QueryPointer, CurrentQuery.QueryLength,
              CurrentQuery.mysql_stmt, qpo->cache_ttl, qpo->timeout, qpo->delay,
              true);
          ...
          // uint64_t
          // hash=client_myds->myconn->local_stmts->compute_hash(current_hostgroup,(char
          // *)client_myds->myconn->userinfo->username,(char
          // *)client_myds->myconn->userinfo->schemaname,(char
          // *)CurrentQuery.QueryPointer,CurrentQuery.QueryLength);
          /*
                                                                          } else
             {
                                                                                  //stmt_info=Session_STMT_Manager->find_prepared_statement_by_hash(hash);
        ...
        case PROCESSING_STMT_EXECUTE: {
          ...
          if (CurrentQuery.stmt_meta)
            if (CurrentQuery.stmt_meta->pkt) {
              uint32_t stmt_global_id = 0;
              memcpy(&stmt_global_id, (char *)(CurrentQuery.stmt_meta->pkt) + 5,
                     sizeof(uint32_t));
              SLDH->reset(stmt_global_id);
        ...
        // if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status &
        // SERVER_STATUS_IN_TRANS)==0)) {
        if (mysql_thread___multiplexing && (myds->myconn->reusable == true) &&
            myds->myconn->IsActiveTransaction() == false &&
            myds->myconn->MultiplexDisabled() == false) {
          myds->DSS = STATE_NOT_INITIALIZED;
          myds->return_MySQL_Connection_To_Pool();
        ...
        if (rc == -1) {
          ...
          */
          if (
              // due to #774 , we now read myconn->server_status instead of
              // myconn->parent->status
              (myconn->server_status ==
               MYSQL_SERVER_STATUS_OFFLINE_HARD) // the query failed because the
                                                 // server is offline hard
              || (myconn->server_status == MYSQL_SERVER_STATUS_SHUNNED &&
                  ...
                  myconn->parent->shunned_and_kill_all_connections ==
                      true) // the query failed because the server is shunned
                            // due to a serious failure
              || (myconn->server_status ==
                  MYSQL_SERVER_STATUS_SHUNNED_REPLICATION_LAG) // slave is
                                                               // lagging! see
                                                               // #774
              ) {
            if (mysql_thread___connect_timeout_server_max) {
            ...
            if (retry_conn) {
              ...
              // previous_status.push(PROCESSING_QUERY);
              switch (status) { // this switch can be replaced with a simple
                                // previous_status.push(status), but it is here
                                // for readibility
              case PROCESSING_QUERY:
                previous_status.push(PROCESSING_QUERY);
                break;
          ...
          if (myerr > 2000) {
            bool retry_conn = false;
            // client error, serious
            proxy_error("Detected a broken connection during query on "
                        "(%d,%s,%d) , FD (Conn:%d , MyDS:%d) : %d, %s\n",
                        myconn->parent->myhgc->hid, myconn->parent->address,
                        myconn->parent->port, myds->fd, myds->myconn->fd, myerr,
                        mysql_error(myconn->mysql));
            // if ((myds->myconn->reusable==true) && ((myds->myprot.prot_status
            ...
            proxy_warning("Error during query on (%d,%s,%d): %d, %s\n",
                          ...
                          mysql_error(myconn->mysql));
            // FIXME: deprecate old MySQL_Result_to_MySQL_wire , not completed
            // yet
            // MySQL_Result_to_MySQL_wire(myconn->mysql,myconn->mysql_result,&client_myds->myprot);
            ...
            case 1053: // Server shutdown in progress
              ...
              if (retry_conn) {
                ...
                // previous_status.push(PROCESSING_QUERY);
                switch (status) { // this switch can be replaced with a simple
                                  // previous_status.push(status), but it is
            ...
            case 1153: // ER_NET_PACKET_TOO_LARGE
              proxy_warning("Error ER_NET_PACKET_TOO_LARGE during query on "
                            "(%d,%s,%d): %d, %s\n",
                            myconn->parent->myhgc->hid, myconn->parent->address,
                            myconn->parent->port, myerr,
                            mysql_error(myconn->mysql));
            ...
            //							MySQL_Result_to_MySQL_wire(myconn->mysql,
            // myconn->MyRS);
            //							CurrentQuery.end();
            //							GloQPro->delete_QP_out(qpo);
            //							qpo=NULL;
          ...
          // start sending to frontend if
          // mysql_thread___threshold_resultset_size is reached
          case 1:
            ...
            break;
          // rc==2 : a multi-resultset (or multi statement) was detected, and
          // the current statement is completed
          case 2:
            MySQL_Result_to_MySQL_wire(myconn->mysql, myconn->MyRS);
            if (myconn->MyRS) { // we also need to clear MyRS, so that the next
                                // staement will recreate it if needed
              delete myconn->MyRS;
              myconn->MyRS = NULL;
            }
  ...
  if (mybe && mybe->server_myds) {
    if (mybe->server_myds->DSS > STATE_MARIADB_BEGIN &&
      ...
      /*
                      ...
                      switch (status) {
                              case FAST_FORWARD:
                                      client_myds->PSarrayOUT->add(pkt.ptr,
         pkt.size); break;
    ...
    *wrong_pass = true;
    ...
    if (client_myds->client_addr) {
      ...
      case AF_INET6: {
        struct sockaddr_in6 *ipv6 =
            (struct sockaddr_in6 *)client_myds->client_addr;
        inet_ntop(client_myds->client_addr->sa_family, &ipv6->sin6_addr, buf,
                  INET6_ADDRSTRLEN);
        client_addr = strdup(buf);
        break;
      }
...
void MySQL_Session::handler___status_CONNECTING_CLIENT___STATE_SERVER_HANDSHAKE(
  ...
  if ((client_myds->myprot.process_pkt_handshake_response(
           (unsigned char *)pkt->ptr, pkt->size) == true) &&
      ((default_hostgroup < 0 && admin == true) ||
       (default_hostgroup >= 0 && admin == false) ||
        ...
        *wrong_pass = true;
        ...
        if (max_connections_reached == true) {
          ...
          proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5,
                      ...
                      client_myds->myconn->userinfo->username, used_users);
          char *a = (char *)"User '%s' has exceeded the 'max_user_connections' "
                            "resource (current value: %d)";
          char *b = (char *)malloc(
              strlen(a) + strlen(client_myds->myconn->userinfo->username) + 16);
        ...
        /*
                                // enable compression
                                if (myconn->options.server_capabilities &
           CLIENT_COMPRESS) { if (myconn->options.compression_min_length) {
...
void MySQL_Session::handler___status_CONNECTING_CLIENT___STATE_SSL_INIT(
  ...
  if (client_myds->myprot.process_pkt_handshake_response(
    ...
    l_free(pkt->size, pkt->ptr);
    // FIXME: this should become close connection
    perror("Hitting a not implemented feature: "
...
void MySQL_Session::
  ...
  if (admin == false) {
    ...
    if (autocommit)
      setStatus += SERVER_STATUS_AUTOCOMMIT;
    client_myds->myprot.generate_pkt_OK(true, NULL, NULL, 1, 0, 0, setStatus, 2,
                                        NULL);
    client_myds->DSS = STATE_SLEEP;
  } else {
    l_free(pkt->size, pkt->ptr);
...
bool MySQL_Session::
  ...
  if (qpo->cache_ttl > 0) {
    ...
    unsigned char *aa = GloQC->get(
        ...
        //			(const unsigned char
        //*)client_myds->mysql_real_query.QueryPtr ,
...
__exit_set_destination_hostgroup:
  ...
  /*
          if (autocommit_on_hostgroup >= 0) {
                  ...
                  // we set current_hostgroup=autocommit_on_hostgroup if
     possible if (transaction_persistent_hostgroup == -1) { if
     (qpo->destination_hostgroup==-1) {
                                  current_hostgroup=autocommit_on_hostgroup;
                          }
                  }
                  autocommit_on_hostgroup=-1;	// at the end, always reset
     autocommit_on_hostgroup to -1
          }
  */
  return false;
}
  ...
  if (admin == false) {
    if (client_myds->myprot.process_pkt_COM_CHANGE_USER(
      ...
      *wrong_pass = true;
      ...
      char *_s =
          (char *)malloc(strlen(client_myds->myconn->userinfo->username) + 100 +
                         strlen(client_addr));
      sprintf(_s,
...
void MySQL_Session::
  ...
  if (mybe->server_myds->myconn == NULL) {
    // we couldn't get a connection for whatever reason, ex: no backends, or too
    // busy
...
void MySQL_Session::MySQL_Stmt_Result_to_MySQL_wire(MYSQL_STMT *stmt,
  ...
  if (stmt_result) {
    MySQL_ResultSet *MyRS = new MySQL_ResultSet(&client_myds->myprot,
                                                stmt_result, stmt->mysql, stmt);
    ...
    if (myerrno == 0) {
      ...
      client_myds->myprot.generate_pkt_OK(
          ...
          mysql->insert_id, mysql->server_status | setStatus,
          mysql->warning_count, mysql->info);
      client_myds->pkt_sid++;
    } else {
      // error
      char sqlstate[10];
...
void MySQL_Session::MySQL_Result_to_MySQL_wire(MYSQL *mysql,
  ...
  if (MyRS) {
    ...
    bool resultset_completed = MyRS->get_resultset(client_myds->PSarrayOUT);
    assert(resultset_completed); // the resultset should always be completed if
                                 // MySQL_Result_to_MySQL_wire is called
    if (transfer_started == false) {   // we have all the resultset when
                                       // MySQL_Result_to_MySQL_wire was called
      if (qpo && qpo->cache_ttl > 0) { // the resultset should be cached
        if (mysql_errno(mysql) == 0) { // no errors
          ...
          GloQC->set(client_myds->myconn->userinfo->hash,
                     //						(const unsigned
                     // char *)client_myds->mysql_real_query.QueryPtr ,
                     //						client_myds->mysql_real_query.QuerySize
                     //,
                     (const unsigned char *)CurrentQuery.QueryPointer,
    ...
    if (myerrno == 0) {
      ...
      if (_myds && _myds->killed_at) { // see case #750
        client_myds->myprot.generate_pkt_ERR(
            true, NULL, NULL, client_myds->pkt_sid + 1, 1907, sqlstate,
            (char *)"Query execution was interrupted, query_timeout exceeded");
      } else {
        client_myds->myprot.generate_pkt_ERR(
            true, NULL, NULL, client_myds->pkt_sid + 1, mysql_errno(mysql),
            sqlstate, mysql_error(mysql));
...
void MySQL_Session::SQLite3_to_MySQL(SQLite3_result *result, char *error,
  ...
  if (result) {
    ...
    for (int i = 0; i < result->columns; i++) {
      myprot->generate_pkt_field(true, NULL, NULL, sid, (char *)"", (char *)"",
                                 (char *)"", result->column_definition[i]->name,
                                 (char *)"", 33, 15, MYSQL_TYPE_VAR_STRING, 1,
                                 0x1f, false, 0, NULL);
      sid++;
    }
...
void MySQL_Session::RequestEnd(MySQL_Data_Stream *myds) {
  ...
  if (qpo) {
    if (qpo->log == 1) {
      GloMyLogger->log_request(this, myds); // we send for logging only if
                                            // logging is enabled for this query
    }
  }
...
// this function tries to report all the memory statistics related to the
// sessions
