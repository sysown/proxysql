...
// This function is called by each thread when it starts. It create a Query
// Processor Table for each thread
...
void Query_Processor::wrunlock() { spin_wrunlock(&rwlock); };
...
QP_rule_t *Query_Processor::new_query_rule(
    ...
    int retries, int delay, int mirror_flagOUT, int mirror_hostgroup,
    char *error_msg, int sticky_conn, int multiplex, int log, bool apply,
    char *comment) {
  ...
  newQR->re_modifiers = 0;
  {
    ...
    for (token = tokenize(&tok); token; token = tokenize(&tok)) {
      if (strncasecmp(token, (char *)"CASELESS", strlen((char *)"CASELESS")) ==
          0) {
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
              ...
              (newQR->negate_match_pattern ? "(!)" : ""), newQR->match_pattern,
              newQR->flagOUT, newQR->replace_pattern,
...
// when commit is called, the version number is increased and the this will
// trigger the mysql threads to get a new Query Processor Table The operation is
// asynchronous
...
SQLite3_result *Query_Processor::get_stats_query_rules() {
  ...
  for (std::vector<QP_rule_t *>::iterator it = rules.begin(); it != rules.end();
       ++it) {
    qr1 = *it;
    if (qr1->active) {
...
SQLite3_result *Query_Processor::get_query_digests() {
  ...
  // for (btree::btree_map<uint64_t, void *>::iterator it=digest_bt_map.begin();
  // it!=digest_bt_map.end(); ++it) {
...
Query_Processor::process_mysql_query(MySQL_Session *sess, void *ptr,
                                     unsigned int size, Query_Info *qi) {
  // to avoid unnecssary deallocation/allocation, we initialize qpo witout new
  // allocation
  ...
  if (__sync_add_and_fetch(&version, 0) > _thr_SQP_version) {
    ...
    proxy_debug(
        ...
        "Detected a changed in version. Global:%d , local:%d . Refreshing...\n",
    ...
    for (std::vector<QP_rule_t *>::iterator it = rules.begin();
      ...
      if (qr1->active) {
        ...
        qr2 = new_query_rule(
            ...
            qr1->mirror_flagOUT, qr1->mirror_hostgroup, qr1->error_msg,
...
__internal_loop:
  for (std::vector<QP_rule_t *>::iterator it = _thr_SQP_rules->begin();
    ...
    if (qr->username && strlen(qr->username)) {
      if (strcmp(qr->username, sess->client_myds->myconn->userinfo->username) !=
          0) {
        proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
    ...
    qr->hits++; // this is done without atomic function because it updates only
                // the local variables
    ...
    if (qr->mirror_flagOUT >= 0) {
      // Note: negative mirror_flagOUT means this rule doesn't change the mirror
      // flagOUT
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
    ...
    if (qr->mirror_hostgroup >= 0) {
      // Note: negative mirror_hostgroup means this rule doesn't change the
      // mirror
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
    ...
    if (qr->multiplex >= 0) {
      ...
      proxy_debug(
          PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
          "query rule %d has set multiplex: %d. Connection will%s multiplex\n",
    ...
    if (qr->replace_pattern) {
      proxy_debug(PROXY_DEBUG_MYSQL_QUERY_PROCESSOR, 5,
                  "query rule %d on match_pattern \"%s\" has a replace_pattern "
                  "\"%s\" to apply\n",
...
__exit_process_mysql_query:
  ...
  l_free(len + 1, query);
  if (sess->mirror ==
      false) { // we process comments only on original queries, not on mirrors
...
// this function is called by mysql_session to free the result generated by
// process_mysql_query()
...
void Query_Processor::query_parser_init(SQP_par_t *qp, char *query,
  ...
  //		libinjection_sqli_init(&qp->sf, query, query_length,
  // FLAG_SQL_MYSQL);
  qp->digest_text = NULL;
  qp->first_comment = NULL;
  qp->query_prefix = NULL;
  // qp->first_comment=(char *)l_alloc(FIRST_COMMENT_MAX_LENGTH);
  // qp->first_comment[0]=0; // initialize it to 0 . Useful to determine if
  // there is any string or not
  if (mysql_thread___query_digests) {
    qp->digest_text = mysql_query_digest_and_first_comment(query, query_length,
                                                           &qp->first_comment);
    qp->digest =
        SpookyHash::Hash64(qp->digest_text, strlen(qp->digest_text), 0);
#ifdef DEBUG
    ...
    // if mysql_thread___query_digests==false but we still want command
    // statistics, we copy the prefix of the query
    if (mysql_thread___commands_stats) {
...
Query_Processor::query_parser_command_type(SQP_par_t *qp) {
...
unsigned long long Query_Processor::query_parser_update_counters(
  ...
  if (sess->CurrentQuery.stmt_info == NULL && qp->digest_text) {
    // this code is executed only if digest_text is not NULL , that means
    // mysql_thread___query_digests was true when the query started
    uint64_t hash2;
    SpookyHash myhash;
    myhash.Init(19, 3);
    assert(sess);
...
enum MYSQL_COM_QUERY_command
Query_Processor::__query_parser_command_type(SQP_par_t *qp) {
  // SQP_par_t *qp=(SQP_par_t *)args;
  char *text = NULL; // this new variable is a pointer to either qp->digest_text
                     // , or to the query
  if (qp->digest_text) {
    text = qp->digest_text;
  } else {
  ...
  case 'a':
  case 'A':
    if (!mystrcasecmp("ALTER",
                      token)) { // ALTER [ONLINE | OFFLINE] [IGNORE] TABLE
      token = (char *)tokenize(&tok);
      if (token == NULL)
        break;
    ...
    if (!mystrcasecmp("ANALYZE",
                      token)) { // ANALYZE [NO_WRITE_TO_BINLOG | LOCAL] TABLE
      token = (char *)tokenize(&tok);
      if (token == NULL)
        break;
