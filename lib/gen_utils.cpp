...
// This function returns a pointer to a substring of the original string. It
// also modifies the original string by setting a null terminator to mark the
...
void PtrArray::operator delete(void *ptr) {
        (((PtrArray *)ptr)->use_l_alloc ? l_free(sizeof(PtrArray), ptr) :
free(ptr));
}
...
PtrArray::~PtrArray() {
        //if (pdata) (use_l_alloc ? l_free(size*sizeof(void *),pdata) :
free(pdata) ); if (pdata) ( free(pdata) );
...
void PtrArray::shrink() {
        ...
        //pdata=(use_l_alloc ? (void **)l_realloc(pdata,new_size*sizeof(void
*),size*sizeof(void *)) : (void **)realloc(pdata,new_size*sizeof(void *)) );
        pdata=(void **)realloc(pdata,new_size*sizeof(void *));
        //pdata=(void **)realloc(pdata,new_size*sizeof(void *));
        //pdata=(void **)l_realloc(pdata,new_size*sizeof(void
*),size*sizeof(void *)); size=new_size;
}
*/
/*
void PtrArray::expand(unsigned int more) {
    if ( (len+more) > size ) {
        unsigned int new_size=l_near_pow_2(len+more);
                                //void *new_pdata=( use_l_alloc ?
l_alloc(new_size*sizeof(void *)) : malloc(new_size*sizeof(void *)) ); void
*new_pdata=malloc(new_size*sizeof(void *));
        ...
        if (pdata) {
            memcpy(new_pdata,pdata,size*sizeof(void *));
                                                //( use_l_alloc ?
l_free(size*sizeof(void *),pdata) : free(pdata) ); free(pdata);
            //free(pdata);
                                                //l_free(size*sizeof(void
*),pdata);
        }
        size=new_size;
...
len, p, s); pdata[len].ptr=p; pdata[len].size=s; len++;
}
...
void PtrSizeArray::remove_index(unsigned int i, PtrSize_t *ps) {
        if (ps) {
                ...
                //fprintf(stderr,"removing from %p of len %d : ptr %p size
