...
void SQLite3_Server::populate_galera_table(MySQL_Session *sess) {
	...
	if (resultset->rows_count==0) {
		...
		ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
...
//			fprintf(stderr,"%d , %s:3306 \n", hg_id , serverid.c_str());
			...
			rc=sqlite3_bind_int64(statement, 1, hg_id); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_text(statement, 2, serverid.c_str(), -1, SQLITE_TRANSIENT); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_int64(statement, 3, 3306); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_int64(statement, 4, 4); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_int64(statement, 5, 0); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_int64(statement, 6, 0); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_int64(statement, 7, 0); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_text(statement, 8, (char *)"NONE", -1, SQLITE_TRANSIENT); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_int64(statement, 9, 0); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_bind_text(statement, 10, (char *)"Primary", -1, SQLITE_TRANSIENT); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			SAFE_SQLITE3_STEP2(statement);
			rc=sqlite3_clear_bindings(statement); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
			rc=sqlite3_reset(statement); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		}
...
void SQLite3_Server::populate_aws_aurora_table(MySQL_Session *sess) {
	...
	for (unsigned int i=0; i<num_aurora_servers[cluster_id]; i++) {
		...
		float cpu = cpu_i;
		cpu /= 100;
		rc=sqlite3_bind_text(statement, 1, serverid.c_str(), -1, SQLITE_TRANSIENT); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		rc=sqlite3_bind_text(statement, 2, sessionid.c_str(), -1, SQLITE_TRANSIENT); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		rc=sqlite3_bind_double(statement, 3, cpu); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		rc=sqlite3_bind_text(statement, 4, lut, -1, SQLITE_TRANSIENT); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		rc=sqlite3_bind_double(statement, 5, lag_ms); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		SAFE_SQLITE3_STEP2(statement);
		rc=sqlite3_clear_bindings(statement); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
		rc=sqlite3_reset(statement); ASSERT_SQLITE3(rc==SQLITE_OK, rc, sessdb);
	}
