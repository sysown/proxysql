...
void MySQLFFTO::report_query_stats(const std::string& query, unsigned long long duration_us, uint64_t affected_rows, uint64_t rows_sent) {
	...
	if (digest_text) {
        ...
        const size_t username_len = ui->username ? strlen(ui->username) : 0;
        myhash.Update(ui->username, username_len);
        myhash.Update(&qp.digest, sizeof(qp.digest));
        const size_t schemaname_len = strlen(schemaname);
        myhash.Update(schemaname, schemaname_len);
        myhash.Update(&m_session->current_hostgroup, sizeof(m_session->current_hostgroup));
        const size_t ca_len = ca ? strlen(ca) : 0;
        myhash.Update(ca, ca_len);
