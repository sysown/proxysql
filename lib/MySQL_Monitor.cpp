...
void * monitor_AWS_Aurora_thread_HG(void *arg) {
	...
	mmsd->interr=0; // reset the value
#ifdef TEST_AURORA
	mmsd->async_exit_status = mysql_query_start(&mmsd->interr, mmsd->mysql, "SELECT SERVER_ID, SESSION_ID, LAST_UPDATE_TIMESTAMP, IF (SERVER_ID = @@AURORA_SERVER_ID, 1, REPLICA_LAG_IN_MILLISECONDS) AS REPLICA_LAG_IN_MILLISECONDS, CPU FROM REPLICA_HOST_STATUS LAST_UPDATE_TIMESTAMP >= DATE_ADD(NOW(), INTERVAL -1 MINUTE) ORDER BY SERVER_ID");
#else
	mmsd->async_exit_status = mysql_query_start(&mmsd->interr, mmsd->mysql, "SELECT SERVER_ID, SESSION_ID, LAST_UPDATE_TIMESTAMP, IF (SESSION_ID = 'MASTER_SESSION_ID', 0, IF (SERVER_ID = @@AURORA_SERVER_ID, 1, REPLICA_LAG_IN_MILLISECONDS)) AS REPLICA_LAG_IN_MILLISECONDS, CPU FROM INFORMATION_SCHEMA.REPLICA_HOST_STATUS WHERE ((REPLICA_LAG_IN_MILLISECONDS >= 0 AND REPLICA_LAG_IN_MILLISECONDS <= 600000) OR SESSION_ID = 'MASTER_SESSION_ID') AND LAST_UPDATE_TIMESTAMP >= DATE_ADD(NOW(), INTERVAL -1 MINUTE) ORDER BY SERVER_ID");
#endif // TEST_AURORA
	while (mmsd->async_exit_status) {
		mmsd->async_exit_status=wait_for_mysql(mmsd->mysql, mmsd->async_exit_status);
		unsigned long long now=monotonic_time();
...
void * monitor_AWS_Aurora_thread(void *arg) {
	...
	mmsd->interr=0; // reset the value
#ifdef TEST_AURORA
	mmsd->async_exit_status = mysql_query_start(&mmsd->interr, mmsd->mysql, "SELECT SERVER_ID, SESSION_ID, LAST_UPDATE_TIMESTAMP, IF (SERVER_ID = @@AURORA_SERVER_ID, 1, REPLICA_LAG_IN_MILLISECONDS) AS REPLICA_LAG_IN_MILLISECONDS, CPU FROM REPLICA_HOST_STATUS WHERE LAST_UPDATE_TIMESTAMP >= DATE_ADD(NOW(), INTERVAL -1 MINUTE) ORDER BY SERVER_ID");
#else
	mmsd->async_exit_status = mysql_query_start(&mmsd->interr, mmsd->mysql, "SELECT SERVER_ID, SESSION_ID, LAST_UPDATE_TIMESTAMP, IF (SESSION_ID = 'MASTER_SESSION_ID', 0, IF (SERVER_ID = @@AURORA_SERVER_ID, 1, REPLICA_LAG_IN_MILLISECONDS)) AS REPLICA_LAG_IN_MILLISECONDS, CPU FROM INFORMATION_SCHEMA.REPLICA_HOST_STATUS LAST_UPDATE_TIMESTAMP >= DATE_ADD(NOW(), INTERVAL -1 MINUTE) ORDER BY SERVER_ID");
#endif // TEST_AURORA
	while (mmsd->async_exit_status) {
		mmsd->async_exit_status=wait_for_mysql(mmsd->mysql, mmsd->async_exit_status);
		unsigned long long now=monotonic_time();
