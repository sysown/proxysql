...
public:
  // ConsumerThreadPing(wqueue<MySQL_Monitor_State_Data*>& queue, void
  // *(*start_routine) (void *), int _n) : m_queue(queue) {
...
MYSQL *MySQL_Monitor_Connection_Pool::get_connection(char *hostname, int port) {
  ...
  if (it != my_connections.end()) {
    ...
    if (!lst->empty()) {
      ...
      size--;
      pthread_mutex_unlock(&mutex);
      memset(ret->net.buff, 0,
             sizeof(unsigned long long)); // reset what was polluted
      return ret;
    }
  }
  pthread_mutex_unlock(&mutex);
...
void MySQL_Monitor_Connection_Pool::put_connection(char *hostname, int port,
  ...
  if (it == my_connections.end()) {
    ...
    my_connections.insert(my_connections.begin(),
                          std::pair<char *, std::list<MYSQL *> *>(buf, lst));
  } else {
    free(buf);
    lst = it->second;
  }
...
MySQL_Monitor::MySQL_Monitor() {
  ...
  admindb->open((char *)"file:mem_admindb?mode=memory&cache=shared",
                SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE |
                    SQLITE_OPEN_FULLMUTEX);
  admindb->execute("ATTACH DATABASE "
                   "'file:mem_monitordb?mode=memory&cache=shared' AS "
                   "'monitor'");
  // define monitoring tables
  tables_defs_monitor = new std::vector<table_def_t *>;
  insert_into_tables_defs(tables_defs_monitor, "mysql_server_connect",
  ...
  insert_into_tables_defs(tables_defs_monitor, "mysql_server_ping",
                          MONITOR_SQLITE_TABLE_MYSQL_SERVER_PING);
  ...
  monitordb->execute("CREATE INDEX IF NOT EXISTS idx_connect_log_time_start ON "
                     "mysql_server_connect_log (time_start_us)");
  monitordb->execute("CREATE INDEX IF NOT EXISTS idx_ping_log_time_start ON "
                     "mysql_server_ping_log (time_start_us)");
  monitordb->execute("CREATE INDEX IF NOT EXISTS idx_read_only_log_time_start "
                     "ON mysql_server_read_only_log (time_start_us)");
  monitordb->execute("CREATE INDEX IF NOT EXISTS "
                     "idx_replication_lag_log_time_start ON "
...
void *monitor_connect_thread(void *arg) {
  ...
  int rc;
  char *query = NULL;
  query = (char *)"INSERT OR REPLACE INTO mysql_server_connect_log VALUES (?1 "
                  ", ?2 , ?3 , ?4 , ?5)";
...
void *monitor_ping_thread(void *arg) {
  ...
  // async_exit_status=mysql_change_user_start(&ret_bool,
  // mysql,"msandbox2","msandbox2","information_schema");
  ...
  while (mmsd->async_exit_status) {
    ...
    if ((mmsd->async_exit_status & MYSQL_WAIT_TIMEOUT) == 0) {
      mmsd->async_exit_status =
          mysql_ping_cont(&mmsd->interr, mmsd->mysql, mmsd->async_exit_status);
    }
  }
...
__exit_monitor_ping_thread:
  mmsd->t2 = monotonic_time();
  {
    ...
    int rc;
    char *query = NULL;
    query = (char *)"INSERT OR REPLACE INTO mysql_server_ping_log VALUES (?1 , "
                    "?2 , ?3 , ?4 , ?5)";
    rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
    assert(rc == SQLITE_OK);
    rc = sqlite3_bind_text(statement, 1, mmsd->hostname, -1, SQLITE_TRANSIENT);
...
bool MySQL_Monitor_State_Data::create_new_connection() {
  ...
  if (use_ssl) {
    mysql_ssl_set(mysql, mysql_thread___ssl_p2s_key,
                  mysql_thread___ssl_p2s_cert, mysql_thread___ssl_p2s_ca, NULL,
  ...
  if (port) {
    myrc = mysql_real_connect(mysql, hostname, mysql_thread___monitor_username,
                              mysql_thread___monitor_password, NULL, port, NULL,
                              0);
  } else {
  ...
  if (myrc == NULL) {
    ...
    // mariadb client library disables NONBLOCK for SSL connections ...
    // re-enable it!
    mysql_options(mysql, MYSQL_OPT_NONBLOCK, 0);
    int f = fcntl(mysql->net.fd, F_GETFL);
#ifdef FD_CLOEXEC
    // asynchronously set also FD_CLOEXEC , this to prevent then when a fork
    // happens the FD are duplicated to new process
...
void *monitor_read_only_thread(void *arg) {
  ...
  mmsd->async_exit_status = mysql_query_start(
      &mmsd->interr, mmsd->mysql, "SHOW GLOBAL VARIABLES LIKE 'read_only'");
  while (mmsd->async_exit_status) {
    mmsd->async_exit_status =
        wait_for_mysql(mmsd->mysql, mmsd->async_exit_status);
    unsigned long long now = monotonic_time();
    if (now > mmsd->t1 + mysql_thread___monitor_read_only_timeout * 1000) {
      ...
      proxy_error("Timeout on read_only check for %s:%d after %lldms. If the "
                  "server is overload, increase "
                  "mysql-monitor_read_only_timeout. Assuming read_only=1\n",
    ...
    if ((mmsd->async_exit_status & MYSQL_WAIT_TIMEOUT) == 0) {
      mmsd->async_exit_status =
          mysql_query_cont(&mmsd->interr, mmsd->mysql, mmsd->async_exit_status);
    }
  }
  mmsd->async_exit_status =
...
__exit_monitor_read_only_thread:
  mmsd->t2 = monotonic_time();
  {
    ...
    int rc;
    char *query = NULL;
    query = (char *)"INSERT OR REPLACE INTO mysql_server_read_only_log VALUES "
                    "(?1 , ?2 , ?3 , ?4 , ?5 , ?6)";
    rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
    assert(rc == SQLITE_OK);
    ...
    if (mmsd->result) {
      ...
      assert(rc == SQLITE_OK);
      //					} else {
      //						rc=sqlite3_bind_null(statement,
      // 5); assert(rc==SQLITE_OK);
      //					}
...
__exit_monitor_replication_lag_thread:
  mmsd->t2 = monotonic_time();
  {
    ...
    query = (char *)"INSERT OR REPLACE INTO mysql_server_replication_lag_log "
                    "VALUES (?1 , ?2 , ?3 , ?4 , ?5 , ?6)";
    rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
    assert(rc == SQLITE_OK);
    //			while (i>0) {
    ...
    if (mmsd->result) {
      ...
      if (j > -1) {
        ...
        if (row) {
          ...
          repl_lag = mysql_thread___monitor_slave_lag_when_null; // new
                                                                 // behavior,
                                                                 // see 669
    ...
    // mmsd->port, (repl_lag==-1 ? 0 : repl_lag));
    MyHGM->replication_lag_action(mmsd->hostgroup_id, mmsd->hostname,
                                  mmsd->port, repl_lag);
    //				delete mmsd;
    //			}
...
void *MySQL_Monitor::monitor_connect() {
  // initialize the MySQL Thread (note: this is not a real thread, just the
  ...
  while (GloMyMon->shutdown == false &&
    ...
    // add support for SSL
    char *query = (char *)"SELECT hostname, port, MAX(use_ssl) use_ssl FROM "
                          "mysql_servers GROUP BY hostname, port";
    unsigned int glover;
    t1 = monotonic_time();
    ...
    if (error) {
      ...
      if (resultset->rows_count) {
        us =
            mysql_thread___monitor_connect_interval / 2 / resultset->rows_count;
      }
  ...
  __end_monitor_connect_loop:
    if (mysql_thread___monitor_enabled == true) {
      ...
      int rc;
      char *query = NULL;
      query = (char *)"DELETE FROM mysql_server_connect_log WHERE "
                      "time_start_us < ?1";
      rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
      assert(rc == SQLITE_OK);
      if (mysql_thread___monitor_history <
          mysql_thread___monitor_ping_interval *
              (mysql_thread___monitor_ping_max_failures + 1)) { // issue #626
      ...
      rc = sqlite3_bind_int64(
          statement, 1,
          time_now - (unsigned long long)mysql_thread___monitor_history * 1000);
      assert(rc == SQLITE_OK);
      SAFE_SQLITE3_STEP(statement);
      rc = sqlite3_clear_bindings(statement);
      assert(rc == SQLITE_OK);
...
void *MySQL_Monitor::monitor_ping() {
  ...
  while (GloMyMon->shutdown == false &&
    ...
    SQLite3_result *resultset = NULL;
    char *query = (char *)"SELECT hostname, port, MAX(use_ssl) use_ssl FROM "
                          "mysql_servers WHERE status NOT LIKE 'OFFLINE\%' "
  ...
  __end_monitor_ping_loop:
    if (mysql_thread___monitor_enabled == true) {
      ...
      char *query = NULL;
      query =
          (char *)"DELETE FROM mysql_server_ping_log WHERE time_start_us < ?1";
      rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
      assert(rc == SQLITE_OK);
      if (mysql_thread___monitor_history <
          mysql_thread___monitor_ping_interval *
    ...
    query = (char *)"SELECT DISTINCT a.hostname, a.port FROM mysql_servers a "
                    "JOIN monitor.mysql_server_ping_log b ON "
                    "a.hostname=b.hostname WHERE status NOT LIKE 'OFFLINE\%' "
                    "AND b.ping_error IS NOT NULL";
    proxy_debug(PROXY_DEBUG_ADMIN, 4, "%s\n", query);
    admindb->execute_statement(query, &error, &cols, &affected_rows,
                               &resultset);
    if (error) {
      ...
      char *new_query = NULL;
      new_query =
          (char *)"SELECT 1 FROM (SELECT hostname,port,ping_error FROM "
                  "mysql_server_ping_log WHERE hostname='%s' AND port='%s' "
                  "ORDER BY time_start_us DESC LIMIT %d) a WHERE ping_error IS "
                  "NOT NULL GROUP BY hostname,port HAVING COUNT(*)=%d";
      for (j = 0; j < i; j++) {
        ...
        if (!error) {
          if (resultset) {
            if (resultset->rows_count) {
              // disable host
              proxy_error("Server %s:%s missed %d heartbeats, shunning it and "
                          "killing all the connections\n",
    ...
    query =
        (char *)"SELECT DISTINCT a.hostname, a.port FROM mysql_servers a JOIN "
                "monitor.mysql_server_ping_log b ON a.hostname=b.hostname "
                "WHERE status NOT LIKE 'OFFLINE\%' AND b.ping_error IS NULL";
    proxy_debug(PROXY_DEBUG_ADMIN, 4, "%s\n", query);
    admindb->execute_statement(query, &error, &cols, &affected_rows,
                               &resultset);
    if (error) {
      ...
      new_query = (char *)"SELECT "
                          "hostname,port,COALESCE(CAST(AVG(ping_success_time_"
                          "us) AS INTEGER),10000) FROM (SELECT "
                          "hostname,port,ping_success_time_us,ping_error FROM "
                          "mysql_server_ping_log WHERE hostname='%s' AND "
                          "port='%s' ORDER BY time_start_us DESC LIMIT 3) a "
                          "WHERE ping_error IS NULL GROUP BY hostname,port";
      for (j = 0; j < i; j++) {
        char *buff = (char *)malloc(strlen(new_query) + strlen(addresses[j]) +
                                    strlen(ports[j]) + 16);
          ...
          if (resultset) {
            if (resultset->rows_count) {
              for (std::vector<SQLite3_row *>::iterator it =
                   ...
                   it != resultset->rows.end(); ++it) {
                SQLite3_row *r = *it; // this should be called just once, but we
...
void *MySQL_Monitor::monitor_read_only() {
  ...
  while (GloMyMon->shutdown == false &&
    ...
    // JOIN mysql_replication_hostgroups ON hostgroup_id=writer_hostgroup OR
    // hostgroup_id=reader_hostgroup WHERE status!='OFFLINE_HARD'";
    // add support for SSL
    char *query = (char *)"SELECT hostname, port, MAX(use_ssl) use_ssl FROM "
                          "mysql_servers JOIN mysql_replication_hostgroups ON "
                          "hostgroup_id=writer_hostgroup OR "
                          "hostgroup_id=reader_hostgroup WHERE status NOT LIKE "
                          "'OFFLINE\%' GROUP BY hostname, port";
    t1 = monotonic_time();
    ...
    if (error) {
      ...
      for (std::vector<SQLite3_row *>::iterator it = resultset->rows.begin();
        ...
        mmsd->mondb = monitordb;
        // pthread_t thr_;
        // if ( pthread_create(&thr_, &attr, monitor_read_only_thread, (void
        // *)mmsd) != 0 ) { 	perror("Thread creation
  ...
  __end_monitor_read_only_loop:
    if (mysql_thread___monitor_enabled == true) {
      ...
      int rc;
      char *query = NULL;
      query = (char *)"DELETE FROM mysql_server_read_only_log WHERE "
                      "time_start_us < ?1";
      rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
      assert(rc == SQLITE_OK);
      if (mysql_thread___monitor_history <
...
void *MySQL_Monitor::monitor_replication_lag() {
  ...
  while (GloMyMon->shutdown == false &&
    ...
    char *error = NULL;
    SQLite3_result *resultset = NULL;
    // char *query=(char *)"SELECT hostgroup_id, hostname, port,
    // max_replication_lag FROM mysql_servers WHERE max_replication_lag > 0 AND
    // status NOT LIKE 'OFFLINE%'";
    // add support for SSL
    char *query =
        (char *)"SELECT hostgroup_id, hostname, port, max_replication_lag, "
                "use_ssl FROM mysql_servers WHERE max_replication_lag > 0 AND "
                "status NOT IN (2,3)";
    t1 = monotonic_time();
    ...
    if (error) {
      ...
      //			sds=(MySQL_Monitor_State_Data
      //**)malloc(resultset->rows_count * sizeof(MySQL_Monitor_State_Data *));
      int us = 100;
      if (resultset->rows_count) {
  ...
  __end_monitor_replication_lag_loop:
    if (mysql_thread___monitor_enabled == true) {
      ...
      int rc;
      char *query = NULL;
      query = (char *)"DELETE FROM mysql_server_replication_lag_log WHERE "
                      "time_start_us < ?1";
      rc = sqlite3_prepare_v2(mondb, query, -1, &statement, 0);
      assert(rc == SQLITE_OK);
      if (mysql_thread___monitor_history <
...
__monitor_run:
  ...
  pthread_create(&monitor_connect_thread, &attr, &monitor_connect_pthread,
                 NULL);
  ...
  pthread_create(&monitor_read_only_thread, &attr, &monitor_read_only_pthread,
                 NULL);
  pthread_t monitor_replication_lag_thread;
  pthread_create(&monitor_replication_lag_thread, &attr,
                 &monitor_replication_lag_pthread, NULL);
