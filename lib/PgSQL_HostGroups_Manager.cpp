...
char** PgSQL_Errors_stats::get_row() {
	...
	pta[7] = strdup(buf);
	snprintf(buf, sizeof(buf), "%lld", static_cast<long long>(first_seen));
	pta[8] = strdup(buf);
	snprintf(buf, sizeof(buf), "%lld", static_cast<long long>(last_seen));
	pta[9] = strdup(buf);
	assert(errmsg);
...
void PgSQL_HostGroups_Manager::unshun_server_all_hostgroups(const char * address, uint16_t port, time_t t, int max_wait_sec, unsigned int *skip_hid) {
	...
	if (GloPTH->variables.hostgroup_manager_verbose >= 3) {
		...
		proxy_info("Calling unshun_server_all_hostgroups() for server %s:%d . Arguments: %lld , %d , %s\n" , address, port, static_cast<long long>(t), max_wait_sec, buf);
	}
	int i, j;
...
void PgSQL_HostGroups_Manager::drop_all_idle_connections() {
	...
	for (i=0; i<(int)MyHostGroups->len; i++) {
		...
		for (j=0; j<(int)myhgc->mysrvs->cnt(); j++) {
			...
			// drop all connections with life exceeding pgsql-connection_max_age , or
			// pgsql-connection_max_lifetime_ms . The latter also applies to connections
			...
			if (pgsql_thread___connection_max_age_ms || pgsql_thread___connection_max_lifetime_ms) {
				unsigned long long curtime=monotonic_time();
				int i=0;
