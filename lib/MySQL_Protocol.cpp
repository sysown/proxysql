...
void unhex_pass(uint8_t *out, const char *in) {
  int i = 0;
  for (i = 0; i < SHA_DIGEST_LENGTH; i++) {
    // this can be simplified a lot, but leaving like this to make it easy to
    // debug
    uint8_t c = 0, d = 0;
...
// see
// http://dev.mysql.com/doc/internals/en/integer.html#packet-Protocol::LengthEncodedInteger
/* arguments to pass:
...
int pkt_end(unsigned char *pkt, unsigned int length, MySQL_Protocol *mp) {
  ...
  //		proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL,1,"End Packet <status:%u
  // warns:%u>\n");
  //	}
...
MySQL_Prepared_Stmt_info::MySQL_Prepared_Stmt_info(unsigned char *pkt,
  ...
  //	fprintf(stderr,"Generating prepared statement with id=%d, cols=%d,
...
// int parse_mysql_pkt(unsigned char *pkt, enum session_states *states, int
// from_client) {
int MySQL_Protocol::parse_mysql_pkt(PtrSize_t *PS_entry,
  ...
  case STATE_NOT_CONNECTED:
    if (from == MYDS_FRONTEND) { // at this stage we expect a packet from the
                                 // server, not from client
      return PKT_ERROR;
    }
    break;
  ...
  case STATE_CLIENT_HANDSHAKE:
    ...
    case OK_Packet:
      ...
      break;
    default:
      return PKT_ERROR; // from the server we expect either an OK or an ERR.
...
// static uint16_t server_capabilities=CLIENT_FOUND_ROWS | CLIENT_PROTOCOL_41 |
// CLIENT_IGNORE_SIGPIPE | CLIENT_TRANSACTIONS | CLIENT_SECURE_CONNECTION |
// CLIENT_CONNECT_WITH_DB | CLIENT_SSL;  static uint8_t server_language=33;
...
// bool send, void **ptr, unsigned int *len) {
bool MySQL_Protocol::generate_statistics_response(bool send, void **ptr,
                                                  unsigned int *len) {
  // FIXME : this function generates a not useful string. It is a placeholder
  // for now

  char buf1[1000];
  unsigned long long t1 = monotonic_time();
  // const char *stats=(char *)"Uptime: 1000  Threads: 1  Questions: 34221015
  // Slow queries: 0  Opens: 757  Flush tables: 1  Open tables: 185  Queries per
  // second avg: 22.289";
  sprintf(buf1, "Uptime: %llu Threads: %d  Questions: %llu  Slow queries: %llu",
...
// bool MySQL_Protocol::generate_pkt_EOF(MySQL_Data_Stream *myds, bool send,
// void **ptr, unsigned int *len, uint8_t sequence_id, uint16_t warnings,
// uint16_t status) {
bool MySQL_Protocol::generate_pkt_EOF(bool send, void **ptr, unsigned int *len,
...
// bool MySQL_Protocol::generate_pkt_ERR(MySQL_Data_Stream *myds, bool send,
// void **ptr, unsigned int *len, uint8_t sequence_id, uint16_t error_code, char
// *sql_state, char *sql_message) {
bool MySQL_Protocol::generate_pkt_ERR(bool send, void **ptr, unsigned int *len,
...
// bool MySQL_Protocol::generate_pkt_OK(MySQL_Data_Stream *myds, bool send, void
// **ptr, unsigned int *len, uint8_t sequence_id, unsigned int affected_rows,
// unsigned int last_insert_id, uint16_t status, uint16_t warnings, char *msg) {
bool MySQL_Protocol::generate_pkt_OK(bool send, void **ptr, unsigned int *len,
  ...
  /*
          ...
          memcpy(_ptr+l, &affected_rows, affected_rows_len); l+=(
     affected_rows_len > 1 ? affected_rows_len - 1 : 1 ); if (last_insert_id_len
     > 1) { _ptr[l]=last_insert_id_prefix; l++;
          }
          memcpy(_ptr+l, &last_insert_id, last_insert_id_len); l+=(
     last_insert_id_len > 1 ? last_insert_id_len -1 : 1 );
  */
...
// bool MySQL_Protocol::generate_pkt_column_count(MySQL_Data_Stream *myds, bool
// send, void **ptr, unsigned int *len, uint8_t sequence_id, uint64_t count) {
bool MySQL_Protocol::generate_pkt_column_count(bool send, void **ptr,
  ...
  /*
          ...
          memcpy(_ptr+l, &count, count_len); l+=( count_len > 1 ? count_len -1 :
     1 );
  */
...
// bool MySQL_Protocol::generate_pkt_field(MySQL_Data_Stream *myds, bool send,
// void **ptr, unsigned int *len, uint8_t sequence_id, char *schema, char
// *table, char *org_table, char *name, char *org_name, uint16_t charset,
// uint32_t column_length, uint8_t type, uint16_t flags, uint8_t decimals, bool
// field_list, uint64_t defvalue_length, char *defvalue) {
bool MySQL_Protocol::generate_pkt_field(
    bool send, void **ptr, unsigned int *len, uint8_t sequence_id, char *schema,
    char *table, char *org_table, char *name, char *org_name, uint16_t charset,
    uint32_t column_length, uint8_t type, uint16_t flags, uint8_t decimals,
    bool field_list, uint64_t defvalue_length, char *defvalue) {
  ...
  _ptr[l] = type;
  l++;
  memcpy(_ptr + l, &flags, sizeof(uint16_t));
  l += sizeof(uint16_t);
  _ptr[l] = decimals;
  l++;
  _ptr[l] = 0x00;
  ...
  // else _ptr[l]=0x00;
  // else fprintf(stderr,"current deflen=%d, defstrlen=%d, namelen=%d,
...
bool MySQL_Protocol::generate_STMT_PREPARE_RESPONSE(
    uint8_t sequence_id, MySQL_STMT_Global_info *stmt_info) {
  ...
  if (stmt_info->num_params) {
    for (i = 0; i < stmt_info->num_params; i++) {
      generate_pkt_field(true, NULL, NULL, sid, (char *)"", (char *)"",
                         ...
                         0, false, 0, NULL); // NOTE: charset is 63 = binary !
      sid++;
    }
    generate_pkt_EOF(
        true, NULL, NULL, sid, 0,
        SERVER_STATUS_AUTOCOMMIT); // FIXME : for now we pass a very broken flag
    sid++;
  }
  if (stmt_info->num_columns) {
    for (i = 0; i < stmt_info->num_columns; i++) {
      MYSQL_FIELD *fd = stmt_info->fields[i];
      // bool MySQL_Protocol::generate_pkt_field(bool send, void **ptr, unsigned
      // int *len, uint8_t sequence_id, char *schema, char *table, char
      // *org_table, char *name, char *org_name, uint16_t charset, uint32_t
      // column_length, uint8_t type, uint16_t flags, uint8_t decimals, bool
      // field_list, uint64_t defvalue_length, char *defvalue) {
      generate_pkt_field(true, NULL, NULL, sid, fd->db, fd->table,
                         fd->org_table, fd->name, fd->org_name, fd->charsetnr,
                         0, fd->type, fd->flags, fd->decimals, false, 0, NULL);
      sid++;
    }
    generate_pkt_EOF(
        true, NULL, NULL, sid, 0,
...
bool MySQL_Protocol::generate_pkt_row(bool send, void **ptr, unsigned int *len,
  ...
  for (col = 0; col < colnums; col++) {
    if (fieldstxt[col]) {
      ...
      l += write_encoded_length_and_string(_ptr + l, fieldslen[col], length_len,
...
uint8_t MySQL_Protocol::generate_pkt_row3(MySQL_ResultSet *myrs,
  ...
  if (pkt.size < (0xFFFFFF + sizeof(mysql_hdr))) {
    ...
    while (left >= (0xFFFFFF + sizeof(mysql_hdr))) {
      ...
      // we are writing a large packet (over 16MB), we assume we are always
      // outside the buffer
...
// bool MySQL_Protocol::generate_pkt_initial_handshake(MySQL_Data_Stream *myds,
// bool send, void **ptr, unsigned int *len) {
bool MySQL_Protocol::generate_pkt_initial_handshake(bool send, void **ptr,
  ...
  *_thread_id = thread_id;
  ...
  //  proxy_create_random_string(myds->myconn->myconn.scramble_buff+0,8,(struct
  //  my_rnd_struct *)&rand_st);
  //#else
  ...
  // create_random_string(mypkt->data+l,12,(struct my_rnd_struct *)&rand_st);
  // l+=12;
  //#ifdef MARIADB_BASE_VERSION
  //  proxy_create_random_string(myds->myconn->myconn.scramble_buff+8,12,(struct
  //  my_rnd_struct *)&rand_st);
  //#else
...
// bool MySQL_Protocol::process_pkt_OK(MySQL_Data_Stream *myds, unsigned char
...
// bool MySQL_Protocol::process_pkt_COM_QUERY(MySQL_Data_Stream *myds, unsigned
...
bool MySQL_Protocol::process_pkt_auth_swich_response(unsigned char *pkt,
  ...
  void *sha1_pass = NULL;
  password = GloMyAuth->lookup((char *)userinfo->username, USERNAME_FRONTEND,
                               &_ret_use_ssl, &default_hostgroup, NULL, NULL,
                               &transaction_persistent, NULL, NULL, &sha1_pass);
  // FIXME: add support for default schema and fast forward , issues #255 and
  // #256
  if (password == NULL) {
    ret = false;
  } else {
    ...
    if (password[0] != '*') { // clear text password
      ...
      if (ret) {
        if (sha1_pass == NULL) {
          // currently proxysql doesn't know any sha1_pass for that specific
          // user, let's set it!
...
bool MySQL_Protocol::process_pkt_COM_CHANGE_USER(unsigned char *pkt,
  ...
  password = GloMyAuth->lookup((char *)user, USERNAME_FRONTEND, &_ret_use_ssl,
                               &default_hostgroup, NULL, NULL,
                               &transaction_persistent, NULL, NULL, &sha1_pass);
  // FIXME: add support for default schema and fast forward, see issue #255 and
  // #256
    ...
    if (pass_len == 0 && strlen(password) == 0) {
      ...
      if (password[0] != '*') { // clear text password
        ...
        if (memcmp(reply, pass, SHA_DIGEST_LENGTH) == 0) {
          ...
          ret =
              proxy_scramble_sha1((char *)pass, (*myds)->myconn->scramble_buff,
                                  password + 1, reply);
          if (ret) {
            if (sha1_pass == NULL) {
              // currently proxysql doesn't know any sha1_pass for that specific
...
// bool MySQL_Protocol::process_pkt_handshake_response(MySQL_Data_Stream *myds,
...
bool MySQL_Protocol::process_pkt_handshake_response(unsigned char *pkt,
  ...
  if (capabilities & CLIENT_COMPRESS) {
    if (myconn->options.server_capabilities & CLIENT_COMPRESS) {
      ...
      // myconn->set_status_compression(true);  // don't enable this here. It
      // needs to be enabled after the OK is sent
    }
  }
#ifdef DEBUG
  if (dump_pkt) {
...
// See https://dev.mysql.com/doc/internals/en/com-stmt-execute.html for
// reference
stmt_execute_metadata_t *
MySQL_Protocol::get_binds_from_pkt(void *ptr, unsigned int size,
  ...
  char *p = (char *)ptr + 5;
  if (*stmt_meta) { // this PS was executed at least once, and we already have
                    // metadata
    ret = *stmt_meta;
  ...
  // FIXME: there is still no free for pkt, so that will be a memory leak that
  // needs to be fixed
  ret->pkt = ptr;
  ...
  if (num_params) {
    ...
    if (new_params_bound_flag) {
      ...
      for (i = 0; i < num_params; i++) {
        ...
        // enum enum_field_types buffer_type=MYSQL_TYPE_DECIMAL; // set a random
        // default
        ...
        lengths[i] = 0;
        //			unsigned long l=0;
        //			uint8_t ll=mysql_decode_length((unsigned char
        //*)p,&l); 			lengths[i]=l; 			p+=ll;
  ...
  *(ret->binds[i].is_null), ret->binds[i].buffer_type);
          }
  #endif
  */
  if (ret)
...
MySQL_ResultSet::MySQL_ResultSet(MySQL_Protocol *_myprot, MYSQL_RES *_res,
  ...
  buffer = NULL;
  if (_stmt ==
      NULL) { // we allocate this buffer only for not prepared statements
  ...
  for (unsigned int i = 0; i < num_fields; i++) {
    ...
    myprot->generate_pkt_field(false, &pkt.ptr, &pkt.size, sid, field->db,
                               ...
                               field->org_name, field->charsetnr, field->length,
                               field->type, field->flags, field->decimals,
                               false, 0, NULL);
    PSarrayOUT->add(pkt.ptr, pkt.size);
    resultset_size += pkt.size;
    sid++;
  ...
  myprot->generate_pkt_EOF(false, &pkt.ptr, &pkt.size, sid, 0,
                           mysql->server_status | setStatus);
  sid++;
  PSarrayOUT->add(pkt.ptr, pkt.size);
  resultset_size += pkt.size;
  if (_stmt) { // binary protocol , we also assume we have ALL the resultset
    ...
    //		MYSQL_BIND *binds=(MYSQL_BIND
    //*)malloc(sizeof(MYSQL_BIND)*column_count);
...
unsigned int MySQL_ResultSet::add_row(MYSQL_ROW row) {
  ...
  if (myprot) {
    sid = myprot->generate_pkt_row3(this, &pkt_length, sid, num_fields, lengths,
                                    row);
  } else {
    unsigned int col = 0;
