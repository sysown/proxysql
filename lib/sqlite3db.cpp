...
bool SQLite3DB::execute(const char *str) {
  ...
  do {
    ...
    if (rc == SQLITE_LOCKED) { // the execution of sqlite3_exec() failed because
                               // locked
  ...
  if (*cols == 0) { // not a SELECT
    *resultset = NULL;
    do {
      ...
      if (rc == SQLITE_LOCKED) { // the execution of the prepared statement
                                 // failed because locked
        usleep(USLEEP_SQLITE_LOCKED);
      }
    } while (rc == SQLITE_LOCKED);
    if (rc == SQLITE_DONE) {
...
int SQLite3DB::check_table_structure(char *table_name, char *table_def) {
  const char *q1 = "SELECT COUNT(*) FROM sqlite_master WHERE type=\"table\" "
                   "AND name=\"%s\" AND sql=\"%s\"";
  int count = 0;
...
void SQLite3DB::rdlock() { spin_wrlock(&rwlock); }

void SQLite3DB::rdunlock() { spin_wrunlock(&rwlock); }

void SQLite3DB::wrlock() { spin_wrlock(&rwlock); }

void SQLite3DB::wrunlock() { spin_wrunlock(&rwlock); }
...
int64_t SQLite3_result::raw_checksum() {
  ...
  for (std::vector<SQLite3_row *>::iterator it = rows.begin(); it != rows.end();
       ++it) {
    SQLite3_row *r = *it;
