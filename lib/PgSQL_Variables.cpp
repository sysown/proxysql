...
pgsql_verify_var PgSQL_Variables::verifiers[SQL_NAME_LAST_HIGH_WM];
pgsql_update_var PgSQL_Variables::updaters[SQL_NAME_LAST_HIGH_WM];
 ...
 *
 * - 'https://dev.pgsql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_character_set_database
 *
...
bool PgSQL_Variables::client_set_value(PgSQL_Session* session, int idx, const std::string& value) {
	...
	case SQL_CHARACTER_ACTION:
		...
		if (value == "1") {
			if (pgsql_variables.client_get_value(session, SQL_CHARACTER_SET)) {
				const char* value = pgsql_variables.client_get_value(session, SQL_CHARACTER_SET);
				uint32_t hash = pgsql_variables.client_get_hash(session, SQL_CHARACTER_SET);

				pgsql_variables.client_set_hash_and_value(session, SQL_CHARACTER_SET_RESULTS, value, hash);
				pgsql_variables.client_set_hash_and_value(session, SQL_CHARACTER_SET_CLIENT, value, hash);
				pgsql_variables.client_set_hash_and_value(session, SQL_CHARACTER_SET_CONNECTION, value, hash);
				pgsql_variables.client_set_hash_and_value(session, SQL_COLLATION_CONNECTION, value, hash);
			}
		}
		// SET CHARSET command from client
			...
			if (pgsql_variables.client_get_value(session, SQL_CHARACTER_SET)) {
				...
				pgsql_variables.client_set_hash_and_value(session, SQL_CHARACTER_SET_CONNECTION, "", 0);
				pgsql_variables.client_set_hash_and_value(session, SQL_COLLATION_CONNECTION, "", 0);
			}
		}
	}
...
bool update_server_variable(PgSQL_Session* session, int idx, int &_rc) {
	...
	if (idx==SQL_CHARACTER_SET_RESULTS) {
		const MARIADB_CHARSET_INFO *ci = NULL;
		ci = proxysql_find_charset_nr(atoi(pgsql_variables.client_get_value(session, SQL_CHARACTER_SET_RESULTS)));

		if (!ci) {
			if (!strcmp(pgsql_variables.client_get_value(session, SQL_CHARACTER_SET_RESULTS), "NULL")) {
			...
			} else if (!strcmp(pgsql_variables.client_get_value(session, SQL_CHARACTER_SET_RESULTS), "binary")) {
				pgsql_variables.server_set_value(session, idx, pgsql_variables.client_get_value(session, idx));
		...
		const MARIADB_CHARSET_INFO *ci = NULL;
		ci = proxysql_find_charset_nr(atoi(pgsql_variables.client_get_value(session, SQL_COLLATION_CONNECTION)));
		...
		const MARIADB_CHARSET_INFO *ci = NULL;
		ci = proxysql_find_charset_nr(atoi(pgsql_variables.client_get_value(session, SQL_CHARACTER_SET_CONNECTION)));
		...
		const MARIADB_CHARSET_INFO *ci = NULL;
		ci = proxysql_find_charset_nr(atoi(pgsql_variables.client_get_value(session, idx)));
		...
		pgsql_variables.server_set_value(session, idx, pgsql_variables.client_get_value(session, idx));
		ret = session->handler_again___status_SETTING_GENERIC_VARIABLE(&_rc, set_var_name, pgsql_variables.server_get_value(session, idx), no_quote, st);
	}
	return ret;
}

bool verify_set_names(PgSQL_Session* session) {
	uint32_t client_charset_hash = pgsql_variables.client_get_hash(session, SQL_CHARACTER_SET_CLIENT);
	if (client_charset_hash == 0)
		return false;

	uint32_t results_charset_hash = pgsql_variables.client_get_hash(session, SQL_CHARACTER_SET_RESULTS);
	if (client_charset_hash != results_charset_hash)
		return false;

	uint32_t connection_charset_hash = pgsql_variables.client_get_hash(session, SQL_CHARACTER_SET_CONNECTION);
	if (client_charset_hash != connection_charset_hash)
		return false;

	uint32_t collation_hash = pgsql_variables.client_get_hash(session, SQL_COLLATION_CONNECTION);
	if (client_charset_hash != collation_hash)
		return false;

	if (client_charset_hash != pgsql_variables.server_get_hash(session, SQL_CHARACTER_SET_CLIENT) ||
			results_charset_hash != pgsql_variables.server_get_hash(session, SQL_CHARACTER_SET_RESULTS) ||
			connection_charset_hash != pgsql_variables.server_get_hash(session, SQL_CHARACTER_SET_CONNECTION) ||
			collation_hash != pgsql_variables.server_get_hash(session, SQL_COLLATION_CONNECTION)) {
		...
		uint32_t hash = pgsql_variables.client_get_hash(session, SQL_CHARACTER_SET_CLIENT);
		const char* value = pgsql_variables.client_get_value(session, SQL_CHARACTER_SET_CLIENT);
		pgsql_variables.server_set_hash_and_value(session, SQL_CHARACTER_SET_CLIENT, value, hash);
		pgsql_variables.server_set_hash_and_value(session, SQL_CHARACTER_SET_RESULTS, value, hash);
		pgsql_variables.server_set_hash_and_value(session, SQL_CHARACTER_SET_CONNECTION, value, hash);
		pgsql_variables.server_set_hash_and_value(session, SQL_COLLATION_CONNECTION, value, hash);
		pgsql_variables.client_set_hash_and_value(session, SQL_CHARACTER_SET, value, hash);
		pgsql_variables.server_set_hash_and_value(session, SQL_CHARACTER_SET, value, hash);
		return true;
	}
