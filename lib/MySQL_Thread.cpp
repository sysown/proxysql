...
static char *mysql_thread_variables_names[] = {
    (char *)"shun_on_failures", (char *)"shun_recovery_time_sec",
    (char *)"query_retries_on_failure", (char *)"connect_retries_on_failure",
    (char *)"connect_retries_delay", (char *)"connection_max_age_ms",
    (char *)"connect_timeout_server", (char *)"connect_timeout_server_max",
    (char *)"eventslog_filename", (char *)"eventslog_filesize",
    (char *)"default_charset", (char *)"free_connections_pct",
    (char *)"session_idle_ms", (char *)"have_compress",
    (char *)"client_found_rows", (char *)"interfaces",
    (char *)"monitor_enabled", (char *)"monitor_history",
    (char *)"monitor_connect_interval", (char *)"monitor_connect_timeout",
    (char *)"monitor_ping_interval", (char *)"monitor_ping_max_failures",
    (char *)"monitor_ping_timeout", (char *)"monitor_read_only_interval",
    (char *)"monitor_read_only_timeout",
    (char *)"monitor_replication_lag_interval",
    (char *)"monitor_replication_lag_timeout", (char *)"monitor_username",
    (char *)"monitor_password", (char *)"monitor_query_interval",
    (char *)"monitor_query_timeout", (char *)"monitor_slave_lag_when_null",
    (char *)"monitor_writer_is_also_reader", (char *)"max_allowed_packet",
    (char *)"max_transaction_time", (char *)"multiplexing",
    //	(char *)"stmt_multiplexing",
    (char *)"enforce_autocommit_on_reads", (char *)"threshold_query_length",
    (char *)"threshold_resultset_size", (char *)"wait_timeout",
    (char *)"max_connections", (char *)"max_stmts_per_connection",
    (char *)"max_stmts_cache", (char *)"default_max_latency_ms",
    (char *)"default_query_delay", (char *)"default_query_timeout",
    (char *)"query_processor_iterations", (char *)"query_processor_regex",
    (char *)"long_query_time", (char *)"query_cache_size_MB",
    (char *)"ping_interval_server_msec", (char *)"ping_timeout_server",
    (char *)"default_schema", (char *)"poll_timeout",
    (char *)"poll_timeout_on_failure", (char *)"server_capabilities",
    (char *)"server_version", (char *)"sessions_sort",
    (char *)"session_idle_show_processlist", (char *)"commands_stats",
    (char *)"query_digests", (char *)"query_digests_lowercase",
    (char *)"servers_stats", (char *)"default_reconnect",
    (char *)"session_debug", (char *)"ssl_p2s_ca", (char *)"ssl_p2s_cert",
    (char *)"ssl_p2s_key", (char *)"ssl_p2s_cipher", (char *)"stacksize",
    (char *)"threads", (char *)"init_connect", NULL};
...
MySQL_Threads_Handler::MySQL_Threads_Handler() {
  ...
  variables.default_max_latency_ms =
      1 * 1000; // by default, the maximum allowed latency for a host is 1000ms
  ...
  //	variables.server_capabilities=CLIENT_FOUND_ROWS | CLIENT_PROTOCOL_41 |
  // CLIENT_IGNORE_SIGPIPE | CLIENT_TRANSACTIONS | CLIENT_SECURE_CONNECTION |
  // CLIENT_CONNECT_WITH_DB | CLIENT_SSL;
  ...
  if (rc > -1) {
    ...
    if (perthrsocks == NULL) {
      for (i = 0; i < num_threads; i++) {
        ...
        while (!__sync_bool_compare_and_swap(&thr->mypolls.pending_listener_add,
                                             0, rc)) {
    ...
    /*
                            while(!__sync_bool_compare_and_swap(&thr->mypolls.pending_listener_change,0,1))
    { cpu_relax_pa(); }
                            while(__sync_fetch_and_add(&thr->mypolls.pending_listener_change,0)==1)
    { cpu_relax_pa(); }
...
void MySQL_Threads_Handler::wrlock() { spin_wrlock(&rwlock); }

void MySQL_Threads_Handler::wrunlock() { spin_wrunlock(&rwlock); }
...
void MySQL_Threads_Handler::commit() {
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_SERVER, 1,
              "Increasing version number to %d - all threads will notice this "
              "and refresh their variables\n",
...
int MySQL_Threads_Handler::get_variable_int(char *name) {
  ...
  //	if (!strcasecmp(name,"stmt_multiplexing")) return
  //(int)variables.stmt_multiplexing;
...
char *MySQL_Threads_Handler::get_variable(
    char *name) { // this is the public function, accessible from admin
...
bool MySQL_Threads_Handler::set_variable(
    char *name,
  ...
  //
  // OUT:
  // false: unable to change the variable value, either because doesn't exist,
  // or because out of range, or read only true: variable value changed
  //
  if (!value)
    return false;
  ...
  if (!strcasecmp(name, "server_version")) {
    if (vallen) {
      ...
      if (strcmp(value, (const char *)"5.1.30") ==
          0) { // per issue #632 , the default 5.1.30 is replaced with 5.5.30
        variables.server_version = strdup((char *)"5.5.30");
      } else {
...
void MySQL_Threads_Handler::init(unsigned int num, size_t stack) {
  ...
  assert(rc == 0);
  mysql_threads = (proxysql_mysql_thread_t *)malloc(
      sizeof(proxysql_mysql_thread_t) * num_threads);
  mysql_threads_idles = (proxysql_mysql_thread_t *)malloc(
      sizeof(proxysql_mysql_thread_t) * num_threads);
}
...
proxysql_mysql_thread_t *MySQL_Threads_Handler::create_thread(
  ...
  if (idles == false) {
    pthread_create(&mysql_threads[tn].thread_id, &attr, start_routine,
                   &mysql_threads[tn]);
  } else {
    pthread_create(&mysql_threads_idles[tn].thread_id, &attr, start_routine,
                   &mysql_threads_idles[tn]);
  }
  return NULL;
}
...
void MySQL_Threads_Handler::start_listeners() {
  ...
  if (strlen(_tmp) == 0) {
    GloMTH->set_variable(
        (char *)"interfaces",
        (char *)"0.0.0.0:6033;/tmp/proxysql.sock"); // set default
  }
  free(_tmp);
...
MySQL_Thread::~MySQL_Thread() {
  ...
  if (myexchange.idle_mysql_sessions) {
    while (myexchange.idle_mysql_sessions->len) {
      MySQL_Session *sess =
          (MySQL_Session *)myexchange.idle_mysql_sessions->remove_index_fast(0);
      delete sess;
    }
  ...
  if (myexchange.resume_mysql_sessions) {
    while (myexchange.resume_mysql_sessions->len) {
      MySQL_Session *sess =
          (MySQL_Session *)myexchange.resume_mysql_sessions->remove_index_fast(
              0);
      delete sess;
    }
  ...
  for (i = 0; i < mypolls.len; i++) {
    if (mypolls.myds[i] && // fix bug #278 . This should be caused by not
                           // initialized datastreams used to ping the backend
...
MySQL_Session *
MySQL_Thread::create_new_session_and_client_data_stream(int _fd) {
  int arg_on = 1;
  ...
  proxy_debug(PROXY_DEBUG_NET, 1,
              "Thread=%p, Session=%p, DataStream=%p -- Created new client Data "
              "Stream\n",
...
// sess->prot.generate_server_handshake(sess->client_myds);
  ...
  // sess->myprot_client.init(&sess->client_myds,
...
bool MySQL_Thread::init() {
  ...
  shutdown = 0;
  my_idle_conns = (MySQL_Connection **)malloc(sizeof(MySQL_Connection *) *
                                              SESSIONS_FOR_CONNECTIONS_HANDLER);
  memset(my_idle_conns, 0,
         sizeof(MySQL_Connection *) * SESSIONS_FOR_CONNECTIONS_HANDLER);
  // my_idle_myds=(MySQL_Data_Stream **)malloc(sizeof(MySQL_Data_Stream
  // *)*SESSIONS_FOR_CONNECTIONS_HANDLER);
  // memset(my_idle_myds,0,sizeof(MySQL_Data_Stream
  // *)*SESSIONS_FOR_CONNECTIONS_HANDLER);
  GloQPro->init_thread();
  refresh_variables();
  i = pipe(pipefd);
...
void MySQL_Thread::run() {
  ...
  //	if (GloMTH->num_threads >= MIN_THREADS_FOR_MAINTENANCE  && this ==
  // GloMTH->mysql_threads[0].worker) {
  ...
  while (shutdown == 0) {
    ...
    if (processing_idles == false &&
      ...
      for (i = 0; i < num_idles; i++) {
        ...
        sess->to_process = 1;
        myds->wait_until =
            curtime + mysql_thread___ping_timeout_server * 1000; // max_timeout
        ...
        //			mypolls.add(POLLIN|POLLOUT, myds->fd, myds,
        // curtime);
      }
  ...
  __run_skip_1:
    ...
    if (idle_maintenance_thread) {
      ...
      while (myexchange.idle_mysql_sessions->len) {
        MySQL_Session *mysess =
            (MySQL_Session *)myexchange.idle_mysql_sessions->remove_index_fast(
                0);
        ...
        // fprintf(stderr,"Adding session %p idx, DS %p idx
    ...
    for (n = 0; n < mypolls.len; n++) {
      ...
      if (myds) {
        // if (GloMTH->num_threads >= MIN_THREADS_FOR_MAINTENANCE &&
          ...
          if (myds->DSS == STATE_SLEEP &&
            ...
            if (_tmp_idle <
              ...
              if (myds->sess->client_myds == myds &&
                  ...
                  (myds->queueOUT.head - myds->queueOUT.tail) ==
                      0) { // extra check
                unsigned int j;
                int conns = 0;
    ...
    if (idle_maintenance_thread == false) {
      ...
      if (shutdown == 0 && thr->shutdown == 0 && idle_mysql_sessions->len) {
        ...
        if (thr->myexchange.idle_mysql_sessions->len) {
          // there are already sessions in the queues. We assume someone already
          // notified worker 0
          empty_queue = false;
        }
  ...
  __run_skip_1a:
    ...
    while ((n = __sync_add_and_fetch(&mypolls.pending_listener_add,
                                     0))) { // spin here
      poll_listener_add(n);
    ...
    if (idle_maintenance_thread) {
      memset(events, 0,
             sizeof(struct epoll_event) *
                 MY_EPOLL_THREAD_MAXEVENTS); // let's make valgrind happy. It
                                             // also seems that needs to be
                                             // zeroed anyway
      // we call epoll()
      ...
      // mypolls.poll_timeout ? mypolls.poll_timeout :
      // mysql_thread___poll_timeout )  );
      proxy_debug(PROXY_DEBUG_NET, 5, "Calling poll with timeout %d\n",
                  (mypolls.poll_timeout
                       ? (mypolls.poll_timeout / 1000 >
                              ...
                              : mysql_thread___poll_timeout)
                       : mysql_thread___poll_timeout));
      // poll is called with a timeout of mypolls.poll_timeout if set , or
      // mysql_thread___poll_timeout
    ...
    while ((n = __sync_add_and_fetch(&mypolls.pending_listener_del,
                                     0))) { // spin here
      poll_listener_del(n);
    ...
    spin_wrlock(&thread_mutex);
    mypolls.poll_timeout = 0; // always reset this to 0 . If a session needs a
                              // specific timeout, it will set this one
      ...
      if (rc) {
        /*
                                        ...
                                        while (epi) {
                                                epi--;
                                                MySQL_Session
           *mysess=(MySQL_Session *)mysql_sessions->index(epi); for (i=0; i<rc;
           i++) { uint32_t sess_thr_id=events[i].data.u32;
                                                        //memcpy(&mysess2,&events[epi].data.ptr,sizeof(MySQL_Session
           *));
                                                        //if (events[i].events
           == EPOLLIN && mysess->thread_session_id==sess_thr_id) { // found it!
                                                        // NOTE: not sure why,
           sometime events returns odd values. If set, we take it out as normal
           worker threads know how to handle it if (events[i].events &&
           mysess->thread_session_id==sess_thr_id) { // found it!
                                                                MySQL_Data_Stream
           *tmp_myds=mysess->client_myds; int dsidx=tmp_myds->poll_fds_idx;
                                                                //fprintf(stderr,"Removing
           session %p, DS %p idx %d\n",mysess,tmp_myds,dsidx);
                                                                mypolls.remove_index_fast(dsidx);
                                                                tmp_myds->mypolls=NULL;
                                                                mysess->thread=NULL;
                                                                // we first
           delete the association in sessmap
                                                                sessmap.erase(mysess->thread_session_id);
                                                                if
           (mysql_sessions->len-1 > 0) {
                                                                        // take
           the last element and adjust the map MySQL_Session
           *mysess_last=(MySQL_Session
           *)mysql_sessions->index(mysql_sessions->len-1);
                                                                        sessmap[mysess_last->thread_session_id]=epi;
                                                                }
                                                                unregister_session(epi);
                                                                resume_mysql_sessions->add(mysess);
                                                                epoll_ctl(efd,
           EPOLL_CTL_DEL, tmp_myds->fd, NULL); i=rc;
                                                        }
                                                }
                                        }
        */
        ...
        for (i = 0; i < rc; i++) {
          if (events[i].data.u32) {
            // NOTE: not sure why, sometime events returns odd values. If set,
            // we take it out as normal worker threads know how to handle it
            if (events[i].events) {
              ...
              // fprintf(stderr,"Removing session %p, DS %p idx
              // %d\n",mysess,tmp_myds,dsidx);
              mypolls.remove_index_fast(dsidx);
              tmp_myds->mypolls = NULL;
              mysess->thread = NULL;
        ...
        /*
                                        ...
                                        for (epi=0; epi<rc; epi++) {
                                                sessindexes[epi]=events[epi].data.fd;
           // is not really a fd! But an integer pointer of mysql_sessions
                                        }
                                        qsort(sessindexes, rc, sizeof(int),
           int_cmp); epi=rc-1; while(epi>-1) { int sessidx=sessindexes[epi]; if
           (sessidx==-1) { // this is the pipe unsigned char c; int
           fd=pipefd[0]; if (read(fd, &c, 1)==-1) {
                                                        }
                                                        maintenance_loop=true;
                                                } else {
                                                        MySQL_Session
           *mysess=(MySQL_Session *)mysql_sessions->index(sessidx);
                                                        MySQL_Data_Stream
           *tmp_myds=mysess->client_myds; int dsidx=tmp_myds->poll_fds_idx;
                                                        mypolls.remove_index_fast(dsidx);
                                                        tmp_myds->mypolls=NULL;
                                                        ...
                                                        resume_mysql_sessions->add(mysess);
                                                        epoll_ctl(efd,
           EPOLL_CTL_DEL, tmp_myds->fd, NULL);
                                                }
                                                epi--;
                                        }
        */
        /*
                                        ...
                                        for (i=0;i<mysql_sessions->len;i++) {
                                                MySQL_Session
           *mysess=(MySQL_Session *)mysql_sessions->index(i); if
           (mysess==myds->sess) { mysess->thread=NULL; unregister_session(i);
                                                //exit_cond=true;
...
#define SESS_TO_SCAN 128
        ...
        if (curtime > (unsigned long long)mysql_thread___wait_timeout * 1000) {
          min_idle =
              curtime - (unsigned long long)mysql_thread___wait_timeout * 1000;
        }
        for (i = 0; i < SESS_TO_SCAN && mysess_idx < mysql_sessions->len; i++) {
          ...
          // if ( (sess_time/1000 > (unsigned long
          // long)mysql_thread___wait_timeout) ) {
    ...
    for (n = 0; n < mypolls.len; n++) {
      ...
      if (myds == NULL) {
        if (mypolls.fds[n].revents) {
          ...
          // this is just the scheleton for issue #253
          if (c) {
            // we are being signaled to sleep for some ms. Before going to sleep
            // we also release the mutex
            spin_wrunlock(&thread_mutex);
            usleep(c * 1000);
            spin_wrlock(&thread_mutex);
            // we enter in maintenance loop only if c is set
            // when threads are signaling each other, there is no need to set
            // maintenance_loop
            maintenance_loop = true;
          }
        }
      ...
      if (mypolls.fds[n].revents == 0) {
        // FIXME: this logic was removed completely because we added mariadb
        // client library. Yet, we need to implement a way to manage connection
        // timeout check for timeout no events. This section is copied from
        ...
        // Note: this logic that was here was removed completely because we
        // added mariadb client library.
        case MYDS_LISTENER:
          ...
          break;
        /*
                                                //Removing from here. Management
           of backend should be done within the session case MYDS_FRONTEND:
                                                        // detected an error on
           backend if ( (mypolls.fds[n].revents & POLLERR) ||
           (mypolls.fds[n].revents & POLLHUP) ) {
                                                                // FIXME: try to
           handle it in a more graceful way myds->sess->set_unhealthy();
                                                        }
                                                        break;
        */
        default:
...
bool MySQL_Thread::process_data_on_data_stream(MySQL_Data_Stream *myds,
  ...
  if (myds->active == FALSE) {
    if (myds->sess->client_myds == myds) {
      ...
      // if this is a backend without fast_forward, do not set unhealthy: it
      // will be handled by client library
...
void MySQL_Thread::process_all_sessions() {
  ...
  bool idle_maintenance_thread = epoll_thread;
  // if (GloMTH->num_threads >= MIN_THREADS_FOR_MAINTENANCE  && this ==
  // GloMTH->mysql_threads[0].worker) { 	idle_maintenance_thread=true;
  //}
  int rc;
  ...
  for (n = 0; n < mysql_sessions->len; n++) {
    ...
    if (sess->mirror == true) { // this is a mirror session
      if (sess->status ==
          WAITING_CLIENT_DATA) { // the mirror session has completed
    ...
    if (sess->healthy == 0) {
      ...
      if (sess->to_process == 1) {
        ...
        if (sess->killed == true) {
          // this is a special cause, if killed the session needs to be executed
          // no matter if paused
          sess->handler();
          unregister_session(n);
          n--;
...
void MySQL_Thread::refresh_variables() {
  ...
  //{
  //		if (GloMyAuth) { // this check if required if GloMyAuth doesn't
  // exist yet
  ...
  //	mysql_thread___stmt_multiplexing=(bool)GloMTH->get_variable_int((char
  //*)"stmt_multiplexing");
...
// there are more than 1 thread . We pause for a little bit to avoid all
// connections to be handled by the same thread
#ifdef SO_REUSEPORT
  ...
  if (c > -1) { // accept() succeeded
    ...
    case AF_INET: {
      ...
      inet_ntop(sess->client_myds->client_addr->sa_family, &ipv4->sin_addr, buf,
                INET_ADDRSTRLEN);
    ...
    // because multiple threads try to handle the same incoming connection, this
    // is OK
  }
}
...
SQLite3_result *MySQL_Threads_Handler::SQL3_Processlist() {
  ...
  for (i = 0;
       i < (mysql_thread___session_idle_show_processlist ? num_threads * 2
                                                         : num_threads);
       i++) {
    if (i < num_threads) {
      thr = (MySQL_Thread *)mysql_threads[i].worker;
...
unsigned int MySQL_Threads_Handler::get_active_transations() {
  ...
  for (i = 0; i < num_threads; i++) {
    if (mysql_threads) {
      ...
      if (thr)
        q +=
            __sync_fetch_and_add(&thr->status_variables.active_transactions, 0);
    }
  }
  return q;
}
