...
void MySQL_Thread::process_all_sessions() {
	...
	for (n=0; n<mysql_sessions->len; n++) {
		...
		auto trace_span = SESSION_TRACE_NAMED(sess, "process_all_sessions__loop_iteration");
...
void MySQL_Thread::listener_handle_new_connection(MySQL_Data_Stream *myds, unsigned int n) {
	...
	if (c>-1) { // accept() succeeded
		...
		auto trace_span = SESSION_TRACE_NAMED(sess, "listener_handle_new_connection__after_create_new_session");
