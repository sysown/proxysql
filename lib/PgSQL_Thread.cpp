...
*/
	...
	for (i = 0; i < PGSQL_NAME_LAST_LOW_WM; i++) {
		const size_t variable_name_len = strlen(pgsql_tracked_variables[i].internal_variable_name);
		const size_t default_variable_name_size = variable_name_len + sizeof("default_");
		char* m = (char*)malloc(default_variable_name_size);
		snprintf(m, default_variable_name_size, "default_%s", pgsql_tracked_variables[i].internal_variable_name);
		ret[fv] = m;
		fv++;
	}
