...
//#define MYHGM_MYSQL_SERVERS "CREATE TABLE mysql_servers ( hostgroup_id INT NOT
// NULL DEFAULT 0, hostname VARCHAR NOT NULL , port INT NOT NULL DEFAULT 3306,
// weight INT CHECK (weight >= 0) NOT NULL DEFAULT 1 , status INT CHECK (status
// IN (0, 1, 2, 3)) NOT NULL DEFAULT 0, PRIMARY KEY (hostgroup_id, hostname,
// port) )"
    ...
    for (i = 0; i < (int)l; i++) {
      ...
      if (myconn->mysql->net.vio) {
        statuses[i] = mysql_change_user_start(
            &ret[i], myconn->mysql, myconn->userinfo->password,
            myconn->userinfo->password, myconn->userinfo->schemaname);
      } else {
        statuses[i] = 0;
        ret[i] = 1;
      }
...
unsigned int MySrvConnList::conns_length() { return conns->len; }
...
MySrvC::MySrvC(char *add, uint16_t p, unsigned int _weight,
               ...
               unsigned int _use_ssl, unsigned int _max_latency_ms,
               char *_comment) {
  address = strdup(add);
  port = p;
  weight = _weight;
...
void MySrvC::connect_error(int err_num) {
  ...
  case 1226: // do not increase counter to avoid Shunning if user has
             // max_user_connections restriction
    return;
    break;
  default:
    break;
  ...
  if (t != time_last_detected_error) {
    ...
    if (__sync_add_and_fetch(&connect_ERR_at_time_last_detected_error, 1) >=
      ...
      MyHGM->wrunlock();
      if (_shu) {
        proxy_error("Shunning server %s:%d with %u errors/sec. Shunning for %u "
                    "seconds\n",
...
MySQL_HostGroups_Manager::MySQL_HostGroups_Manager() {
  ...
  mydb = new SQLite3DB();
  mydb->open((char *)"file:mem_mydb?mode=memory&cache=shared",
...
bool MySQL_HostGroups_Manager::server_add(
  ...
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
              ...
              "with weight %u , status %u, %s compression, max_connections %d, "
              "max_replication_lag %u, use_ssl=%u, max_latency_ms=%u\n",
              add, p, hid, _weight, status,
              (_comp ? "with" : "without") /*, _charset */, _max_connections,
              _max_replication_lag, _use_ssl, _max_latency_ms);
  char *q = (char *)"INSERT INTO mysql_servers_incoming VALUES (%u, \"%s\", "
                    "%u, %u, %u, %u, %u, %u, %u, %u, '%s')";
  ...
  sprintf(query, q, hid, add, p, _weight, status, _comp /*,_charset */,
          _max_connections, _max_replication_lag, _use_ssl, _max_latency_ms, o);
...
bool MySQL_HostGroups_Manager::commit() {
  ...
  wrlock();
  query =
      (char *)"SELECT mem_pointer, t1.hostgroup_id, t1.hostname, t1.port FROM "
              "mysql_servers t1 LEFT OUTER JOIN mysql_servers_incoming t2 ON "
              "(t1.hostgroup_id=t2.hostgroup_id AND t1.hostname=t2.hostname "
              "AND t1.port=t2.port) WHERE t2.hostgroup_id IS NULL";
  mydb->execute_statement(query, &error, &cols, &affected_rows, &resultset);
  if (error) {
    ...
    for (std::vector<SQLite3_row *>::iterator it = resultset->rows.begin();
      ...
      proxy_warning("Removed server at address %lld, hostgroup %s, address %s "
                    "port %s. Setting status OFFLINE HARD and immediately "
                    "dropping all free connections. Used connections will be "
                    "dropped when trying to use them\n",
                    ptr, r->fields[1], r->fields[2], r->fields[3]);
      MySrvC *mysrvc = (MySrvC *)ptr;
      mysrvc->status = MYSQL_SERVER_STATUS_OFFLINE_HARD;
      //__sync_fetch_and_sub(&status.server_connections_connected,
      // mysrvc->ConnectionsFree->conns->len);
  ...
  //	proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4, "INSERT OR IGNORE INTO
  // mysql_servers(hostgroup_id, hostname, port, weight, status, compression,
  // max_connections) SELECT hostgroup_id, hostname, port, weight, status,
  // compression, max_connections FROM mysql_servers_incoming\n");
  mydb->execute("INSERT OR IGNORE INTO mysql_servers(hostgroup_id, hostname, "
                "port, weight, status, compression, max_connections, "
                "max_replication_lag, use_ssl, max_latency_ms, comment) SELECT "
                "hostgroup_id, hostname, port, weight, status, compression, "
                "max_connections, max_replication_lag, use_ssl, "
                "max_latency_ms, comment FROM mysql_servers_incoming");

  // SELECT FROM mysql_servers whatever is not identical in
  // mysql_servers_incoming, or where mem_pointer=0 (where there is no pointer
  // yet)
  query =
      (char *)"SELECT t1.*, t2.weight, t2.status, t2.compression, "
              "t2.max_connections, t2.max_replication_lag, t2.use_ssl, "
              "t2.max_latency_ms, t2.comment FROM mysql_servers t1 JOIN "
              "mysql_servers_incoming t2 ON (t1.hostgroup_id=t2.hostgroup_id "
              "AND t1.hostname=t2.hostname AND t1.port=t2.port) WHERE "
              "mem_pointer=0 OR t1.weight<>t2.weight OR t1.status<>t2.status "
              "OR t1.compression<>t2.compression OR "
              "t1.max_connections<>t2.max_connections OR "
              "t1.max_replication_lag<>t2.max_replication_lag OR "
              "t1.use_ssl<>t2.use_ssl OR t1.max_latency_ms<>t2.max_latency_ms "
  ...
  if (error) {
    ...
    for (std::vector<SQLite3_row *>::iterator it = resultset->rows.begin();
         it != resultset->rows.end(); ++it) {
      SQLite3_row *r = *it;
      long long ptr = atoll(r->fields[11]); // increase this index every time a
                                            // new column is added
      ...
      if (ptr == 0) {
        ...
        MySrvC *mysrvc = new MySrvC(
            ...
            atoi(r->fields[9]), r->fields[10]); // add new fields here if adding
                                                // more columns in mysql_servers
        proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
        ...
        if (atoi(r->fields[6]) != atoi(r->fields[15])) {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      "Changing max_connections for server %s:%d (%s:%d) from "
                      "%d (%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[6], mysrvc->max_connections,
                      atoi(r->fields[15]));
        ...
        if (atoi(r->fields[8]) != atoi(r->fields[17])) {
          proxy_debug(
              PROXY_DEBUG_MYSQL_CONNPOOL, 5,
              "Changing use_ssl for server %s:%d (%s:%d) from %d (%d) to %d\n",
              mysrvc->address, mysrvc->port, r->fields[1], atoi(r->fields[2]),
              r->fields[8], mysrvc->use_ssl, atoi(r->fields[17]));
        ...
        if (atoi(r->fields[9]) != atoi(r->fields[18])) {
          proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 5,
                      ...
                      "%d (%d) to %d\n",
                      mysrvc->address, mysrvc->port, r->fields[1],
                      atoi(r->fields[2]), r->fields[9], mysrvc->max_latency_us,
                      atoi(r->fields[18]));
...
void MySQL_HostGroups_Manager::generate_mysql_servers_table() {
  for (unsigned int i = 0; i < MyHostGroups->len; i++) {
    ...
    for (unsigned int j = 0; j < myhgc->mysrvs->servers->len; j++) {
      ...
      char *q = (char *)"INSERT INTO mysql_servers "
                        "VALUES(%d,\"%s\",%d,%d,%d,%u,%u,%u,%u,%u,'%s',%llu)";
      char *o = escape_string_single_quotes(mysrvc->comment, false);
      char *query =
          (char *)malloc(strlen(q) + 8 + strlen(mysrvc->address) + 8 + 8 + 8 +
                         8 + 8 + 16 + 8 + 16 + 32 + strlen(o));
      sprintf(query, q, mysrvc->myhgc->hid, mysrvc->address, mysrvc->port,
              mysrvc->weight, mysrvc->status, mysrvc->compression,
              mysrvc->max_connections, mysrvc->max_replication_lag,
              mysrvc->use_ssl, mysrvc->max_latency_us / 1000, o, ptr);
      ...
      fprintf(stderr,
              ...
              "max_connections: %u , max_replication_lag: %u , use_ssl: %u , "
              "max_latency_ms: %u , comment: %s\n",
              mysrvc->myhgc->hid, mysrvc->address, mysrvc->port, mysrvc->weight,
              st, mysrvc->max_connections, mysrvc->max_replication_lag,
              mysrvc->use_ssl, mysrvc->max_latency_us * 1000, mysrvc->comment);
      proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4, "%s\n", query);
      // fprintf(stderr,"%s\n",query);
      mydb->execute(query);
      free(query);
...
SQLite3_result *MySQL_HostGroups_Manager::dump_table_mysql_servers() {
  ...
  int affected_rows = 0;
  SQLite3_result *resultset = NULL;
  char *query = (char *)"SELECT hostgroup_id, hostname, port, weight, CASE "
                        "status WHEN 0 THEN \"ONLINE\" WHEN 1 THEN \"SHUNNED\" "
                        "WHEN 2 THEN \"OFFLINE_SOFT\" WHEN 3 THEN "
                        "\"OFFLINE_HARD\" WHEN 4 THEN \"SHUNNED\" END, "
                        "compression, max_connections, max_replication_lag, "
                        "use_ssl, max_latency_ms, comment FROM mysql_servers";
  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4, "%s\n", query);
  mydb->execute_statement(query, &error, &cols, &affected_rows, &resultset);
  wrunlock();
  return resultset;
...
MySQL_HostGroups_Manager::dump_table_mysql_replication_hostgroups() {
  ...
  int affected_rows = 0;
  SQLite3_result *resultset = NULL;
  char *query = (char *)"SELECT writer_hostgroup, reader_hostgroup, comment "
...
// MyHGC * MySQL_HostGroups_Manager::MyConn_add_to_pool(MySQL_Connection *c, int
// _hid) {
void MySQL_HostGroups_Manager::push_MyConn_to_pool(MySQL_Connection *c,
  ...
  // if (c->largest_query_length > (unsigned
  // int)mysql_thread___threshold_query_length) {
  ...
  if (mysrvc->status == MYSQL_SERVER_STATUS_ONLINE) {
    if (c->async_state_machine == ASYNC_IDLE) {
      // if (c->local_stmts->get_num_entries() >
      // mysql_thread___max_stmts_per_connection) {
      if (c->local_stmts->get_num_entries() >
          (unsigned int)GloMTH->variables.max_stmts_per_connection) {
        proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                    "Destroying MySQL_Connection %p, server %s:%d with status "
                    "%d because has too many prepared statements\n",
                    c, mysrvc->address, mysrvc->port, mysrvc->status);
        delete c;
      } else {
        c->optimize();
...
MySrvC *MyHGC::get_random_MySrvC() {
  ...
  if (l) {
    ...
    for (j = 0; j < l; j++) {
      mysrvc = mysrvs->idx(j);
      if (mysrvc->status ==
          MYSQL_SERVER_STATUS_ONLINE) { // consider this server only if ONLINE
        if (mysrvc->ConnectionsUsed->conns_length() <
            mysrvc->max_connections) { // consider this server only if didn't
                                       // reach max_connections
          if (mysrvc->current_latency_us <
              (mysrvc->max_latency_us
                   ? mysrvc->max_latency_us
                   : mysql_thread___default_max_latency_ms *
                         1000)) { // consider the host only if not too far
            sum += mysrvc->weight;
        ...
        if (mysrvc->status == MYSQL_SERVER_STATUS_SHUNNED) {
          ...
          if (mysrvc->shunned_automatic &&
            ...
            time_t t;
            t = time(NULL);
            // we do all these changes without locking . We assume the server is
            // not used from long even if the server is still in used and any of
            // the follow command fails it is not critical because this is only
            // an attempt to recover a server that is probably dead anyway
              ...
              if ((mysrvc->shunned_and_kill_all_connections ==
                  ...
                  || (mysrvc->shunned_and_kill_all_connections == true &&
                      ...
                      mysrvc->ConnectionsFree->conns_length() ==
                          0) // if shunned_and_kill_all_connections is set,
                             // ensure all connections are already dropped
                  ) {
    ...
    if (sum == 0) {
      // per issue #531 , we try a desperate attempt to bring back online any
      // shunned server we do this lowering the maximum wait time to 10% most of
      // the follow code is copied from few lines above
      time_t t;
      t = time(NULL);
      int max_wait_sec =
          (mysql_thread___shun_recovery_time_sec * 1000 >=
                   mysql_thread___connect_timeout_server_max
               ? mysql_thread___connect_timeout_server_max / 100 - 1
    ...
    for (j = 0; j < l; j++) {
      ...
      if (mysrvc->status ==
        ...
        if (len < mysrvc->max_connections) { // consider this server only if
                                             // didn't reach max_connections
          if (mysrvc->current_latency_us <
              (mysrvc->max_latency_us
                   ? mysrvc->max_latency_us
                   : mysql_thread___default_max_latency_ms *
    ...
    /*
                    i=rand()%l;
                    mysrvc=mysrvs->idx(i);
                    proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7, "Returning MySrvC
       %p, server %s:%d\n", mysrvc, mysrvc->address, mysrvc->port); return
       mysrvc;
    */
  }
...
unsigned int MySrvList::cnt() { return servers->len; }
...
MySQL_Connection *
MySQL_HostGroups_Manager::get_MyConn_from_pool(unsigned int _hid) {
  MySQL_Connection *conn = NULL;
  wrlock();
...
void MySQL_HostGroups_Manager::destroy_MyConn_from_pool(MySQL_Connection *c) {
  ...
  if (mysrvc->status == MYSQL_SERVER_STATUS_ONLINE && c->send_quit &&
      queue.size() < 100) {
    // overall, the backend seems healthy and so it is the connection. Try to
    // reset it
    proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
                "Trying to reset MySQL_Connection %p, server %s:%d\n", c,
                mysrvc->address, mysrvc->port);
    to_del = false;
    c->userinfo->set(mysql_thread___monitor_username,
                     mysql_thread___monitor_password,
                     mysql_thread___default_schema, NULL);
    queue.add(c);
  } else {
    // we lock only this part of the code because we need to remove the
    // connection from ConnectionsUsed
    wrlock();
    proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7,
    ...
    for (j = 0; j < (int)myhgc->mysrvs->cnt(); j++) {
      ...
      if (mysql_thread___connection_max_age_ms) {
        ...
        for (i = 0; i < (int)mscl->conns_length(); i++) {
          ...
          if (curtime >
              mc->creation_time + mysql_thread___connection_max_age_ms * 1000) {
            mc = mscl->remove(0);
            delete mc;
            i--;
          }
  ...
  for (i = 0; i < (int)MyHostGroups->len; i++) {
    ...
    for (j = 0; j < (int)myhgc->mysrvs->cnt(); j++) {
      ...
      while (mysrvc->ConnectionsFree->conns_length() &&
        ...
        // *)mysrvc->ConnectionsFree->conns->remove_index_fast(0);
        MySQL_Connection *conn = mysrvc->ConnectionsFree->remove(0);
        delete conn;
        //__sync_fetch_and_sub(&status.server_connections_connected, 1);
      }
...
void MySQL_HostGroups_Manager::read_only_action(char *hostname, int port,
                                                int read_only) {
  // define queries
  // const char *Q1=(char *)"SELECT hostgroup_id FROM mysql_servers join
  // mysql_replication_hostgroups ON hostgroup_id=writer_hostgroup WHERE
  // hostname='%s' AND port=%d AND status=0"; // this query run against myhgm DB
  // . status is an INTEGER
  const char *Q1 =
      (char *)"SELECT hostgroup_id,status FROM mysql_replication_hostgroups "
              "JOIN mysql_servers ON hostgroup_id=writer_hostgroup AND "
              "hostname='%s' AND port=%d";
  // const char *Q1B=(char *)"SELECT hostgroup_id,status FROM
  // mysql_replication_hostgroups LEFT JOIN mysql_servers ON
  // hostgroup_id=writer_hostgroup AND hostname='%s' AND port=%d";
  const char *Q1B =
      (char *)"SELECT hostgroup_id,status FROM ( SELECT DISTINCT "
              "writer_hostgroup FROM mysql_replication_hostgroups JOIN "
              "mysql_servers WHERE (hostgroup_id=writer_hostgroup OR "
              "reader_hostgroup=hostgroup_id) AND hostname='%s' AND port=%d ) "
              "LEFT JOIN mysql_servers ON hostgroup_id=writer_hostgroup AND "
              "hostname='%s' AND port=%d";
  const char *Q2 =
      (char *)"UPDATE OR IGNORE mysql_servers SET hostgroup_id=(SELECT "
              "writer_hostgroup FROM mysql_replication_hostgroups WHERE "
              "reader_hostgroup=mysql_servers.hostgroup_id) WHERE "
              "hostname='%s' AND port=%d AND hostgroup_id IN (SELECT "
              "reader_hostgroup FROM mysql_replication_hostgroups WHERE "
              "reader_hostgroup=mysql_servers.hostgroup_id)";
  const char *Q3A =
      (char *)"INSERT OR IGNORE INTO mysql_servers(hostgroup_id, hostname, "
              "port, status, weight, max_connections, max_replication_lag, "
              "use_ssl, max_latency_ms, comment) SELECT reader_hostgroup, "
              "hostname, port, status, weight, max_connections, "
              "max_replication_lag, use_ssl, max_latency_ms, "
              "mysql_servers.comment FROM mysql_servers JOIN "
              "mysql_replication_hostgroups ON "
              "mysql_servers.hostgroup_id=mysql_replication_hostgroups.writer_"
              "hostgroup WHERE hostname='%s' AND port=%d";
  const char *Q3B =
      (char *)"DELETE FROM mysql_servers WHERE hostname='%s' AND port=%d AND "
              "hostgroup_id IN (SELECT reader_hostgroup FROM "
              "mysql_replication_hostgroups WHERE "
              "reader_hostgroup=mysql_servers.hostgroup_id)";
  const char *Q4 =
      (char *)"UPDATE OR IGNORE mysql_servers SET hostgroup_id=(SELECT "
              "reader_hostgroup FROM mysql_replication_hostgroups WHERE "
              "writer_hostgroup=mysql_servers.hostgroup_id) WHERE "
              "hostname='%s' AND port=%d AND hostgroup_id IN (SELECT "
              "writer_hostgroup FROM mysql_replication_hostgroups WHERE "
              "writer_hostgroup=mysql_servers.hostgroup_id)";
  const char *Q5 = (char *)"DELETE FROM mysql_servers WHERE hostname='%s' AND "
                           "port=%d AND hostgroup_id IN (SELECT "
                           "writer_hostgroup FROM mysql_replication_hostgroups "
  ...
  case 0:
    if (num_rows == 0) {
      // the server has read_only=0 , but we can't find any writer, so we
      // perform a swap
      ...
      // there is a server in writer hostgroup, let check the status of present
      // and not present hosts this is the same query as Q1, but with a LEFT
      // JOIN
      ...
      for (std::vector<SQLite3_row *>::iterator it = resultset->rows.begin();
           it != resultset->rows.end(); ++it) {
        SQLite3_row *r = *it;
        int status = MYSQL_SERVER_STATUS_OFFLINE_HARD; // default status, even
                                                       // for missing
      ...
      if (act == true) { // there are servers either missing, or with
                         // stats=OFFLINE_HARD
        GloAdmin->mysql_servers_wrlock();
        GloAdmin->save_mysql_servers_runtime_to_database(
            false); // SAVE MYSQL SERVERS FROM RUNTIME
  ...
  case 1:
    if (num_rows) {
      // the server has read_only=1 , but we find it as writer, so we perform a
      // swap
      GloAdmin->mysql_servers_wrlock();
      GloAdmin->save_mysql_servers_runtime_to_database(
          false); // SAVE MYSQL SERVERS FROM RUNTIME
...
// it temporary disables a host that is not responding to pings, and mark the
// host in a way that when used the connection will be dropped
...
// the connection pool will use this information to evaluate or exclude a
// specific hosts note that this variable is in microsecond, while user defines
// it in millisecond
    ...
    if (l) {
      for (j = 0; j < l; j++) {
        ...
        for (k = 0; k < mysrvc->ConnectionsFree->conns_length(); k++) {
          // MySQL_Connection *myconn=(MySQL_Connection
          // *)mysrvc->ConnectionsFree->conns->index(k);
