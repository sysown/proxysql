...
ProxySQL_GlobalVariables::ProxySQL_GlobalVariables() {
  ...
  opt->add((const char *)"~/proxysql.cnf", 0, 1, 0,
           ...
           (const char *)"--config");
  // opt->add((const char *)"",0,0,0,(const char *)"Enable custom memory
  // allocator",(const char *)"-m",(const char *)"--custom-memory");
...
// apply settings from cmdline, that have priority over config file
  ...
  //	if (GloVars.__cmd_proxysql_foreground>=0) {
  // foreground=GloVars.__cmd_proxysql_foreground; }
