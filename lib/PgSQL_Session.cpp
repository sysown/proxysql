...
void PgSQL_Session::reset_all_backends() {
	PgSQL_Backend* mybe;
	while (mybes->len) {
		mybe = (PgSQL_Backend*)mybes->remove_index_fast(0);
		mybe->reset();
		delete mybe;
	}
};
 ...
 */
bool PgSQL_Session::handler_special_queries_STATUS(PtrSize_t* pkt) {
...
bool PgSQL_Session::handler_again___verify_backend_autocommit() {
	if (sending_set_autocommit) {
		...
		// and we return
		//
		// Nonetheless, we need to set autocommit in backend's PgSQL_Connection
		PgSQL_Connection* mc = mybe->server_myds->myconn;
		mc->set_autocommit(autocommit);
		mc->options.last_set_autocommit = (mc->options.autocommit ? 1 : 0);
		if (autocommit == 1) {
			// due to  MySQL bug https://bugs.pgsql.com/bug.php?id=107875 related to
		...
		if (autocommit == false) { // also IsAutoCommit==false
			if (mysql_thread___enforce_autocommit_on_reads == false) {
				if (
					...
					&&
					// another edge case related to MySQL bug https://bugs.pgsql.com/bug.php?id=107875
