...
void MySQL_Logger::wrunlock() { spin_wrunlock(&rwlock); };
...
void MySQL_Logger::open_log_unlocked() {
  ...
  // int fd=open(filen, O_WRONLY | O_APPEND | O_CREAT , S_IRUSR | S_IWUSR |
  // S_IRGRP | S_IWGRP);
  free(filen);
  // close(fd);
};
...
void MySQL_Logger::log_request(MySQL_Session *sess, MySQL_Data_Stream *myds) {
  ...
  // std::cout << " " << curtime_real << " " << curtime_mono << " " <<
  // sess->CurrentQuery.end_time << std::endl;
  int cl = 0;
