...
void PgSQL_Authentication::print_version() {
		fprintf(stderr,"Standard PgSQL Authentication rev. %s -- %s -- %s\n", PGSQL_AUTHENTICATION_VERSION, __FILE__, __TIMESTAMP__);
	};
...
unsigned int PgSQL_Authentication::memory_usage() {
	unsigned int ret=0;
#ifdef PROXYSQL_AUTH_PTHREAD_MUTEX
...
int PgSQL_Authentication::increase_frontend_user_connections(char *username, int *mc) {
	uint64_t hash1, hash2;
...
void PgSQL_Authentication::decrease_frontend_user_connections(char *username) {
	uint64_t hash1, hash2;
	SpookyHash *myhash=new SpookyHash();
	myhash->Init(1,2);
	myhash->Update(username,strlen(username));
...
bool PgSQL_Authentication::del(char * username, enum cred_username_type usertype, bool set_lock) {
	bool ret=false;
	uint64_t hash1, hash2;
	SpookyHash *myhash=new SpookyHash();
	myhash->Init(1,2);
...
bool PgSQL_Authentication::set_SHA1(char * username, enum cred_username_type usertype, void *sha_pass) {
	bool ret=false;
	uint64_t hash1, hash2;
	SpookyHash *myhash=new SpookyHash();
	myhash->Init(1,2);
...
bool PgSQL_Authentication::exists(char * username) {
	bool ret = false;
	uint64_t hash1, hash2;
	SpookyHash myhash;
	myhash.Init(1,2);
...
uint64_t PgSQL_Authentication::get_runtime_checksum() {
...
void PgSQL_Authentication::save_pgsql_users(unique_ptr<SQLite3_result>&& users) {
	this->pgsql_users_resultset = std::move(users);
}

SQLite3_result* PgSQL_Authentication::get_current_pgsql_users() {
	return this->pgsql_users_resultset.get();
}
