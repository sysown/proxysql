...
uint64_t KV_BtreeArray::get_data_size() {
  uint64_t r =
      __sync_fetch_and_add(&Glo_num_entries, 0) *
      (sizeof(QC_entry_t) + sizeof(QC_entry_t *) * 2 +
       sizeof(uint64_t) * 2); // +  __sync_fetch_and_add(&Glo_size_values,0) ;
  return r;
};
...
void KV_BtreeArray::purge_some(unsigned long long QCnow_ms, bool aggressive) {
  ...
  if (aggressive == false) {
    ...
    total_freeable_memory =
        freeable_memory + ret * (sizeof(QC_entry_t) + sizeof(QC_entry_t *) * 2 +
                                 sizeof(uint64_t) * 2);
  ...
  if (aggressive || cond_freeable_memory) {
    ...
    if (aggressive) {
      access_ms_lower_mark =
          access_ms_min + (access_ms_max - access_ms_min) *
                              0.1; // hardcoded for now. Remove the entries with
                                   // access time in the 10% range closest to
                                   // access_ms_min
    }
    spin_wrlock(&lock);
    for (i = 0; i < ptrArray->len; i++) {
      ...
      if (__sync_fetch_and_add(&qce->ref_count, 0) <=
          1) { // currently not in use
        if (qce->expire_ms == EXPIRE_DROPIT ||
            qce->expire_ms < QCnow_ms) { // legacy algorithm
          drop_entry = true;
        }
        if (aggressive) { // we have been asked to do aggressive purging
          if (drop_entry == false) { // if the entry is already marked to be
                                     // dropped, no further check
    ...
    if (removed_entries) {
      ...
      // removed_entries, __sync_fetch_and_sub(&Glo_num_entries,0),
      // ptrArray.len);
      //				if (removed_entries)
      // firintf(stderr,"Size of  KVBtreeArray:%d , freed_memory:%lu,
      // Glo_cntGet:%lu, Glo_cntGetOK:%lu, Glo_cntSet:%lu, cntPurge:%lu,
      // dataIN:%lu, dataOUT:%lu\n", cnt() , Glo_total_freed_memory, Glo_cntGet,
      // Glo_cntGetOK, Glo_cntSet, Glo_cntPurge, Glo_dataIN, Glo_dataOUT);
    }
  }
};
...
bool Query_Cache::set(uint64_t user_hash, const unsigned char *kp, uint32_t kl,
                      unsigned char *vp, uint32_t vl,
                      unsigned long long curtime_ms,
