...
MySQL_Connection::MySQL_Connection() {
  ...
  local_stmts =
      new MySQL_STMTs_local(false); // false by default, it is a backend
};
...
MySQL_Connection::~MySQL_Connection() {
  ...
  //	// FIXME: with the use of mysql client library , this part should be
  // gone.
  //	// for now only commenting it to be sure it is not needed
  //	if (myds) {
  //		myds->shut_hard();
  //	} else {
  //		proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 4, "MySQL_Connection %p
  //,  fd:%d\n", this, fd); 		shutdown(fd, SHUT_RDWR);
  //		close(fd);
  //	}
  if (MyRS) {
    delete MyRS;
...
void MySQL_Connection::store_result_cont(short event) {
  proxy_debug(PROXY_DEBUG_MYSQL_PROTOCOL, 6, "event=%d\n", event);
  async_exit_status =
      mysql_store_result_cont(&mysql_result, mysql, mysql_status(event, true));
}
...
MDB_ASYNC_ST MySQL_Connection::handler(short event) {
  unsigned int processed_bytes = 0; // issue #527 : this variable will store the
                                    // amount of bytes processed during this
                                    // event
  if (mysql == NULL) {
...
handler_again:
  ...
  case ASYNC_CONNECT_END:
    if (!ret_mysql) {
      ...
      proxy_error("Failed to mysql_real_connect() on %s:%d , FD (Conn:%d , "
                  "MyDS:%d) , %d: %s.\n",
  ...
  case ASYNC_CONNECT_TIMEOUT:
    ...
    // parent->address, parent->port, myds->sess->thread->curtime ,
    // myds->wait_until, myds->sess->thread->curtime - myds->wait_until);
  ...
  case ASYNC_USE_RESULT_CONT:
    ...
    if (async_exit_status) {
      ...
      if (mysql_row) {
        ...
        processed_bytes += br; // issue #527 : this variable will store the
                               // amount of bytes processed during this event
        if (processed_bytes >
...
int MySQL_Connection::async_query(short event, char *stmt, unsigned long length,
  ...
  assert(mysql);
  assert(ret_mysql);
  server_status = parent->status; // we copy it here to avoid race condition.
                                  // The caller will see this
  if ((server_status == MYSQL_SERVER_STATUS_OFFLINE_HARD) // the server is
                                                          // OFFLINE as specific
                                                          // by the user
      ...
      || (server_status ==
          MYSQL_SERVER_STATUS_SHUNNED_REPLICATION_LAG) // slave is lagging! see
                                                       // #774
      ) {
    return -1;
  }
  switch (async_state_machine) {
  case ASYNC_QUERY_END:
    processing_multi_statement = false; // no matter if we are processing a
                                        // multi statement or not, we reached
                                        // the end
    return 0;
    break;
  case ASYNC_IDLE:
...
void MySQL_Connection::ProcessQueryAndSetStatusFlags(char *query_digest_text) {
  if (query_digest_text == NULL)
    return;
  if (get_status_user_variable() ==
      false) { // we search for variables only if not already set
    ...
    if (strcasecmp(
            query_digest_text,
            "SET SQL_SAFE_UPDATES=?,SQL_SELECT_LIMIT=?,MAX_JOIN_SIZE=?") == 0) {
      set_status_user_variable(true);
    }
  }
  if (get_status_prepared_statement() ==
      false) { // we search if prepared was already executed
    if (!strncasecmp(query_digest_text, "PREPARE ", strlen("PREPARE "))) {
      set_status_prepared_statement(true);
    }
  }
  if (get_status_temporary_table() ==
      false) { // we search for temporary if not already set
    if (!strncasecmp(query_digest_text, "CREATE TEMPORARY TABLE ",
                     strlen("CREATE TEMPORARY TABLE "))) {
      set_status_temporary_table(true);
    }
  }
  if (get_status_lock_tables() ==
      false) { // we search for lock tables only if not already set
  ...
  if (get_status_lock_tables() ==
    ...
    if (!strncasecmp(query_digest_text, "FLUSH TABLES WITH READ LOCK",
                     strlen("FLUSH TABLES WITH READ LOCK"))) { // issue 613
      set_status_lock_tables(true);
    }
  }
  ...
  if (get_status_get_lock() ==
      false) { // we search for get_lock if not already set
...
void MySQL_Connection::optimize() {
  if (mysql->net.max_packet > 65536) { // FIXME: temporary, maybe for very long
                                       // time . This needs to become a global
                                       // variable
...
// this function is identical to async_query() , with the only exception that
// MyRS should never be set
int MySQL_Connection::async_send_simple_command(short event, char *stmt,
  ...
  assert(ret_mysql);
  if ((parent->status ==
       MYSQL_SERVER_STATUS_OFFLINE_HARD) // the server is OFFLINE as specific by
                                         // the user
  ...
  if (MyRS) {
    ...
    proxy_error("Retrieved a resultset while running a simple command. This is "
                "an error!! Simple command: %s\n",
                stmt);
  }
  if (async_state_machine == ASYNC_QUERY_END) {
    if (mysql_errno(mysql)) {
