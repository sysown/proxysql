...
MySQL_Authentication::MySQL_Authentication() {
  ...
  //	authdb->open((char *)"file:mem_authdb?mode=memory&cache=shared",
...
// bool MySQL_Authentication::user_add(unsigned int hid, char *add, uint16_t p,
// unsigned int _weight, enum MySerStatus status, unsigned int _comp /*, uint8_t
// _charset */, unsigned int _max_connections) {
//  bool ret;
//  proxy_debug(PROXY_DEBUG_MYSQL_CONNPOOL, 7, "Adding in mysql_servers_incoming
//  server %s:%d in hostgroup %u with weight %u , status %u, %s compression,
//  max_connections %d\n", add,p,hid,_weight,status, (_comp ? "with" :
//  "without") /*, _charset */ , _max_connections); char *q=(char *)"INSERT INTO
//  mysql_servers_incoming VALUES (%u, \"%s\", %u, %u, %u, %u, %u)"; char
//  *query=(char *)malloc(strlen(q)+strlen(add)+100);
//  sprintf(query,q,hid,add,p,_weight,status,_comp /*,_charset */,
//  _max_connections); ret=mydb->execute(query); free(query); return ret;
//}
...
__loop_remove_inactives:
  for (i = 0; i < cg.cred_array->len; i++) {
    ...
    if (ado->__active == false) {
      ...
      goto __loop_remove_inactives; // we aren't sure how the underlying
                                    // structure changes, so we jump back to 0
    }
  }
  spin_wrunlock(&cg.lock);
}
...
int MySQL_Authentication::dump_all_users(account_details_t ***ads) {
  ...
  //	for (it=creds_frontends.bt_map.begin();
  // it!=creds_frontends.bt_map.end(); it++) { 		account_details_t
  //*ad=lookup->second; 		ads[idx]=(account_details_t
  //*)malloc(sizeof(account_details_t));
  //	}
