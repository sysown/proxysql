...
MySQL_STMTs_local::~MySQL_STMTs_local() {
  ...
  for (std::map<uint32_t, MYSQL_STMT *>::iterator it = m.begin(); it != m.end();
       ++it) {
...
MySQL_STMT_Manager::find_prepared_statement_by_stmt_id(uint32_t id, bool lock) {
...
MySQL_STMT_Global_info *
MySQL_STMT_Manager::find_prepared_statement_by_hash(uint64_t hash, bool lock) {
  MySQL_STMT_Global_info *ret = NULL; // assume we do not find it
  if (lock) {
    spin_wrlock(&rwlock);
  }
