...
MySQL_Data_Stream::~MySQL_Data_Stream() {
  ...
  if (fd > 0) {
    //	// Changing logic here. The socket should be closed only if it is not a
    // backend
    if (myds_type == MYDS_FRONTEND) {
      ...
      proxy_debug(
          ...
          this, myconn, (myconn ? "not reusable" : "is empty"));
      shut_hard();
      //		shutdown(fd,SHUT_RDWR);
      //		close(fd);
    }
  }
  // Commenting the follow line of code and adding an assert. We should ensure
  // that if a myconn exists it should be removed *before*
...
void MySQL_Data_Stream::init() {
  if (myds_type != MYDS_LISTENER) {
    ...
    //		if (PSarrayOUTpending==NULL) PSarrayOUTpending= new
    // PtrSizeArray();
    if (resultset == NULL)
...
// TODO: should check the status of the data stream, and identify if it is safe
// to reconnect or if the session should be destroyed
...
void MySQL_Data_Stream::check_data_flow() {
  if ((PSarrayIN->len || queue_data(queueIN)) &&
    ...
    // there is data at both sides of the data stream: this is considered a
    // fatal error
    proxy_error("Session=%p, DataStream=%p -- Data at both ends of a MySQL "
                "data stream: IN <%d bytes %d packets> , OUT <%d bytes %d "
                "packets>\n",
                sess, this, PSarrayIN->len, queue_data(queueIN),
                PSarrayOUT->len, queue_data(queueOUT));
    shut_soft();
  }
  // if ((myds_type==MYDS_BACKEND) && (myconn->myconn.net.fd==0) && (revents &
  // POLLOUT)) {
...
int MySQL_Data_Stream::write_to_net() {
  ...
  // bytes_io = ( encrypted ? SSL_write (ssl, queue_r_ptr(queueOUT), s) :
  // send(fd, queue_r_ptr(queueOUT), s, 0) );
  ...
  if (bytes_io < 0) {
    if (encrypted == false) {
      if (mypolls->fds[poll_fds_idx].revents &
          POLLOUT) { // in write_to_net_poll() we has remove this safety
...
int MySQL_Data_Stream::buffer2array() {
  ...
  if (myconn->get_status_compression() == true) {
    if ((queueIN.pkt.size == 0) && queue_data(queueIN) >= 7) {
      ...
      queueIN.pkt.ptr = l_alloc(queueIN.pkt.size);
      memcpy(queueIN.pkt.ptr, &queueIN.hdr,
  ...
  if ((queueIN.pkt.size > 0) && (queueIN.pkt.size == queueIN.partial)) {
    if (myconn->get_status_compression() == true) {
      ...
      while (progress < datalength) {
        ...
        //				memcpy(&_a,_ptr+progress,sizeof(mysql_hdr));
        //				unsigned int
        // size=_a.pkt_length+sizeof(mysql_hdr);
        //				unsigned char *ptrP=(unsigned char
        //*)l_alloc(size);
...
#define MAX_COMPRESSED_PACKET_SIZE 10 * 1024 * 1024
  ...
  while (i < PSarrayOUT->len && total_size < MAX_COMPRESSED_PACKET_SIZE) {
    ...
    total_size += p->size;
    //	The following 7 lines of code are commented as they seem responsible for
    // bug reported in
  ...
  if (total_size <= MAX_COMPRESSED_PACKET_SIZE) {
    ...
    // if we reach here, it means we have one single packet larger than
    ...
    if (queueOUT.partial == 0) { // read a new packet
      if (PSarrayOUT->len - idx) {
        ...
        if (myconn->get_status_compression() == true) {
          ...
          generate_compressed_packet(); // it is copied directly into
                                        // queueOUT.pkt
        } else {
          VALGRIND_DISABLE_ERROR_REPORTING;
          ...
          VALGRIND_ENABLE_ERROR_REPORTING;
          // this is a special case, needed because compression is enabled
          // *after* the first OK
...
int MySQL_Data_Stream::myds_connect(char *address, int connect_port,
  ...
  if (myconn == NULL)
    myconn = new MySQL_Connection(); // FIXME: why here? // 20141011 /// should
                                     // be removed
...
int MySQL_Data_Stream::assign_fd_from_mysql_conn() {
  assert(myconn);
  // proxy_debug(PROXY_DEBUG_MYSQL_CONNECTION, 5, "Sess=%p, myds=%p, oldFD=%d,
