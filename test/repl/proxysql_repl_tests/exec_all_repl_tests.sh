...
# run exec_repl_test.sh for each mysql version with/without ssl, pass $2 which might be 'no_binlog_checksum'
...
do
  ...
  do
    ...
    if [ "$mysql_ver" == "5.6" ] && [ "$ssl_par" == "ssl" ]; then
      ...
      ./exec_repl_test.sh "$mysql_ver" "$ssl_par" "" "$2"
      ...
      ./exec_repl_test.sh "$mysql_ver" "$ssl_par" "$debezium" "$2"
