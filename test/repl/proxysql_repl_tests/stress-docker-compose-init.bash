...
echo "Initializing CI Infra '${INFRA}' mode '${DOCKER_MODE}' for stress test..."
...
if [[ ${DEBEZIUM} = "debezium" ]]; then
	docker-compose --profile stress-mysql --profile debezium up -d
else
  docker-compose --profile stress-mysql up -d
fi

# pass 'no_binlog_checksum' if it's required
./bin/stress-mysql-post.bash $1
./bin/stress-proxy-post.bash
