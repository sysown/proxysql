...
echo "[$(date)] Configuring replication: mysql1(source)=>proxysql=>mysql3/4/5(replicas) ..."
...
do
  ...
  then
    echo
    echo "[ERROR] Timeout of $TIMEOUT_LIMIT seconds reached while waiting for replication on ${MYSQL3_HOST}${INFRA} to be configured"
    exit 1
  fi
  sleep 1
  printf "."
...
do
  ...
  then
    echo
    echo "[ERROR] Timeout of $TIMEOUT_LIMIT seconds reached while waiting for replication on ${MYSQL4_HOST}${INFRA} to be configured"
    exit 1
  fi
  sleep 1
  printf "."
...
do
  ...
  then
    echo
    echo "[ERROR] Timeout of $TIMEOUT_LIMIT seconds reached while waiting for replication on ${MYSQL5_HOST}${INFRA} to be configured"
    exit 1
  fi
  sleep 1
  printf "."
