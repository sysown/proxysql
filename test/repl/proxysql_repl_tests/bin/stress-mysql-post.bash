...
while [[ ! $(mysql ${SSLOPT} -h${MYSQL4_HOST}${INFRA} -P${MYSQL4_PORT} -uroot -proot -e 'SELECT version()\G' 2>/dev/null) =~ version ]]; do echo -n '.'; sleep 1; done;
echo " got $(mysql ${SSLOPT} -h${MYSQL4_HOST}${INFRA} -P${MYSQL4_PORT} -uroot -proot -e 'SELECT version()\G' 2>/dev/null | grep version)"
...
while [[ ! $(mysql ${SSLOPT} -h${MYSQL5_HOST}${INFRA} -P${MYSQL5_PORT} -uroot -proot -e 'SELECT version()\G' 2>/dev/null) =~ version ]]; do echo -n '.'; sleep 1; done;
echo " got $(mysql ${SSLOPT} -h${MYSQL5_HOST}${INFRA} -P${MYSQL5_PORT} -uroot -proot -e 'SELECT version()\G' 2>/dev/null | grep version)"
...
while [[ ! $(mysql ${SSLOPT} -h${MYSQL1_HOST}${INFRA} -P${MYSQL1_PORT} -uroot -proot -e 'SHOW SLAVE HOSTS;' 2>&1 | grep -v 'Using a password' | wc -l) -eq 5 ]]; do echo -n '.'; sleep 1; done;
