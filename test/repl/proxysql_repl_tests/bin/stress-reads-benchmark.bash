...
MYSQL_PWD=root
# set read/write ratio: 100 reads / 5 writes
RANGE_SELECTS=off
POINT_SELECTS=100
...
sysbench_run="sysbench /usr/share/sysbench/$SCRIPT --table-size=$SIZE_TABLES --tables=$NUM_TABLES --threads=$RUN_THREADS \
 ...
 --range_selects=$RANGE_SELECTS --point_selects=$POINT_SELECTS \
...
if [[ ! -z "$1" ]]; then
  ...
  echo "[`date '+%Y-%m-%d %H:%M:%S'`] Running sysbench read-writes ratio: 100/5 against proxysql"
  echo "Logging output to: "$1/$log_file
  for i in $(seq 1 10)
  do
