...
main() {
    ...
    exec_admin_silent "DELETE FROM mcp_query_rules WHERE rule_id BETWEEN 190 AND 199;" >/dev/null 2>&1
    ...
    run_test "T11.1: pgsql target_id rule blocks run_sql_readonly" \
        bash -c "echo \"${resp_block_target}\" | grep -q '\"isError\":true' && echo \"${resp_block_target}\" | grep -qi 'Phase11: pg target rule matched'"
    ...
    run_test "T11.2: pgsql username rule blocks run_sql_readonly" \
        bash -c "echo \"${resp_block_user}\" | grep -q '\"isError\":true' && echo \"${resp_block_user}\" | grep -qi 'Phase11: pg username rule matched'"
    ...
    run_test "T11.3: pgsql run_sql_readonly executes on target" \
        bash -c "! echo \"${resp_ok}\" | grep -q '\"isError\":true'"
    ...
    run_test "T11.4: pgsql explain_sql executes on target" \
        bash -c "! echo \"${resp_explain}\" | grep -q '\"isError\":true'"

    local stats_190
    stats_190=$(exec_admin_silent "SELECT username || '|' || target_id FROM stats_mcp_query_rules WHERE rule_id=190;")
    run_test "T11.5: stats include target_id for pgsql target rule" \
        bash -c "[ \"${stats_190}\" = \"|${pg_target_id}\" ]"

    local stats_191
    stats_191=$(exec_admin_silent "SELECT username || '|' || target_id FROM stats_mcp_query_rules WHERE rule_id=191;")
