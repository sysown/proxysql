...
int main() {
...

	{
		...
		mgr.refresh_runtime_views_for_query(
			"SELECT * FROM runtime_mysqlx_users WHERE active=1", nullptr, nullptr, nullptr);
		...
		mgr.refresh_runtime_views_for_query(
			"SELECT * FROM mysql_users", nullptr, nullptr, nullptr);
		ok(users_fires.load() == 2 && routes_fires.load() == 1,
		...
		mgr.refresh_runtime_views_for_query(
			"SELECT * FROM RUNTIME_MYSQLX_USERS", nullptr, nullptr, nullptr);
...

	{
		...
		mgr.refresh_runtime_views_for_query(
			"SELECT * FROM runtime_mysqlx_users_extra", nullptr, nullptr, nullptr);
		ok(fires.load() == 0,
		   "longer-identifier overlap does not match (fires=%d)", fires.load());

		// Suffix overlap.
		mgr.refresh_runtime_views_for_query(
			"SELECT * FROM stats_runtime_mysqlx_users", nullptr, nullptr, nullptr);
		ok(fires.load() == 0,
		   "left-side identifier prefix does not match (fires=%d)", fires.load());

		// Embedded inside a longer word, no boundary on either side.
		mgr.refresh_runtime_views_for_query(
			"SELECT 'xruntime_mysqlx_usersy' FROM dual", nullptr, nullptr, nullptr);
		ok(fires.load() == 0,
		...
		mgr.refresh_runtime_views_for_query(
			"DESC runtime_mysqlx_users", nullptr, nullptr, nullptr);
		ok(fires.load() == 2,
		   "identifier at end-of-string still matches (fires=%d)", fires.load());

		// Identifier at start of string.
		mgr.refresh_runtime_views_for_query(
			"runtime_mysqlx_users", nullptr, nullptr, nullptr);
		ok(fires.load() == 3,
