...
int main() {
	...
	L1.unlink(ln);
	L2.push_back(ln);
	ok(o2.next == &ln && ln.prev == &o2, "unlinked node can move to a different owner");
	L1.unlink(ln);
	...
	ok(empty_lists.empty(99), "unknown hid is empty");
	ok(empty_lists.head(99) == nullptr, "unknown hid head is null");

	PgSQL_Waiter_Lists fifo;
	PgSQL_Waiter_Node f1;
	PgSQL_Waiter_Node f2;
	PgSQL_Waiter_Node f3;
	f1.hid = 7;
	f2.hid = 7;
	f3.hid = 7;
	fifo.push_back(f1);
	fifo.push_back(f2);
	fifo.push_back(f3);
	ok(fifo.head(7) == &f1 && f1.next == &f2 && f2.next == &f3, "FIFO oldest is head");
	fifo.unlink(f1);
	ok(fifo.head(7) == &f2, "after serving head, next oldest is head");
	fifo.push_back(f1);
	ok(fifo.head(7) == &f2 && f3.next == &f1 && f1.prev == &f3 && f1.next == nullptr, "re-enter waiter goes to tail");

	ok(!vanilla_pool_checkout(false, "", -1), "vanilla false when gtid_uuid is empty string");
	ok(vanilla_pool_checkout(false, nullptr, -1) && !vanilla_pool_checkout(true, "x", 5), "vanilla conjunction");
