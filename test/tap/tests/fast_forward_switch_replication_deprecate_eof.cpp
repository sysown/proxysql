...
int main() {
    ...
    diag("Credentials Info:");
    diag("  Proxy Host: %s:%d, User: %s, Pass: %s", cl.host, cl.port, cl.root_username, cl.root_password);
    diag("  Admin Host: %s:%d, User: %s, Pass: %s", cl.admin_host, cl.admin_port, cl.admin_username, cl.admin_password);

    // 1. Generate a large binlog file - MUST connect via ProxySQL
    MYSQL* proxysql_conn = mysql_init(NULL);
    diag("Connecting to ProxySQL for data generation at %s:%d as %s/%s", cl.host, cl.port, cl.root_username, cl.root_password);
    ...
    diag("Connecting to ProxySQL Admin at %s:%d as %s/%s", cl.host, cl.admin_port, cl.admin_username, cl.admin_password);
