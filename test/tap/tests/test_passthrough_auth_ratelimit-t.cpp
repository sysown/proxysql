...
int main() {
	...
	/*
	 * Snapshot original use_ssl for MYSQL8_HG so we can restore it
	 * in cleanup. See the long comment in e2e-t.cpp for rationale.
	 */
	int saved_use_ssl = 0;
	{
		char q[256];
	...
	/* Restore use_ssl for MYSQL8_HG (see e2e-t.cpp for rationale). */
	{
		char q[256];
		snprintf(q, sizeof(q),
