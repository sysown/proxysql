...
int main(int argc, char** argv) {
	...
	MYSQL_QUERY(mysql, "set @@session.sql_big_selects=1");
	...
	MYSQL_QUERY(mysql, "set @@session.max_join_size=10000000");
	MYSQL_QUERY(mysql, "select 1");
	result = mysql_store_result(mysql);
	mysql_free_result(result);
	...
	// Case 3 : setting max_join_size to max value should not reset sql_big_selects to OFF
	MYSQL_QUERY(mysql, "set @@session.sql_big_selects=1");
	MYSQL_QUERY(mysql, "select 1");
	result = mysql_store_result(mysql);
	mysql_free_result(result);
	...
	show_variable(mysql, "sql_big_selects", value);
	ok(value == "ON", "SQL_BIG_SELECTS should be initialized to ON. Actual value [%s]", value.c_str()); // ok_3

	MYSQL_QUERY(mysql, "set @@session.max_join_size=18446744073709551615");
	MYSQL_QUERY(mysql, "select 1");
	result = mysql_store_result(mysql);
	mysql_free_result(result);

	show_variable(mysql, "sql_big_selects", value);
	ok(value == "ON", "Case 3 : setting max_join_size to max value should not reset sql_big_selects to OFF. SQL_BIG_SELECTS should be initialized to ON. Actual value [%s]", value.c_str()); // ok_4

	// Case 4 : setting sql_big_selects for unchanged NOT maximum max_join_size
	MYSQL_QUERY(mysql, "set @@session.max_join_size=1844677");
	MYSQL_QUERY(mysql, "select 1");
	result = mysql_store_result(mysql);
	mysql_free_result(result);
	...
	MYSQL_QUERY(mysql, "set @@session.sql_big_selects=0");
	MYSQL_QUERY(mysql, "select 1");
	result = mysql_store_result(mysql);
	mysql_free_result(result);
