/**
 * @file test_pgsql_query_digests_stages-t.cpp
 * @brief This test file is responsible for checking the correctness of
 *   'query_digest_and_first_comment_2' implementation for PostgreSQL mode.
 * @details This is a PostgreSQL-specific version of the digest testing framework.
 *   The test allows to individually execute three types of testing payloads:
 *   * Regular PostgreSQL tokenizer digests: These payloads types can be found in the 'pgsql_quoted_identifiers_digests.hjson'
 *     file. This file can be used for testing the correctness of the tokenizer behavior for PostgreSQL-specific payload
 *     and configuration settings, using a simple JSON format. The `hjson` file allows comments starting with '//'.
 *   * PostgreSQL quoted identifier tests: These payloads test PostgreSQL-specific quoted identifier handling
 *     to ensure proper preservation of quoted identifiers in digests.
 ...
 *     are left as documentation and regression testing, the main file of these
 ...
 *     * 'postgres': Only executes PostgreSQL-specific quoted identifier tests.
 *
 *   So:
 *     * `test_pgsql_query_digests_stages-t regular` will just execute the regular tests.
 *     * `test_pgsql_query_digests_stages-t postgres` will only execute PostgreSQL-specific tests.
 *     * `test_pgsql_query_digests_stages-t regular,postgres,crashing` will execute all tests.
 *
 *   By default all types of tests are executed.
 *
 * @note This file is specifically for PostgreSQL testing and should NOT modify MySQL digest behavior.
 */
#include <cstring>
#include <iostream>
#include <fstream>
