...
int insert_and_check(MYSQL_STMT *stmti, MYSQL_STMT *stmts, int id, char *name1, MYSQL_TIME *ts1, int *i1, char *name2, MYSQL_TIME *ts2, int *i2) {
	...
	bindi[7].buffer_type= MYSQL_TYPE_STRING;
	bindi[7].buffer= name1;
	if (name1) {
		name1l = strlen(name1);
		bindi[7].buffer_length= name1l;
		bindi[7].is_null= 0;
		bindi[7].length= &name1l;
	} else {
		bindi[7].is_null= &is_null_on;
	}

	bindi[8].buffer_type= MYSQL_TYPE_TIME;
	bindi[8].buffer= (char *)ts1;
	if (ts1) {
		bindi[8].is_null= 0;
	} else {
		bindi[8].is_null= &is_null_on;
	}	
	bindi[8].length= 0;

	bindi[9].buffer_type= MYSQL_TYPE_LONG;
	bindi[9].buffer= (char *)i1;
	if (i1) {
		bindi[9].is_null= 0;
	} else {
		bindi[9].is_null= &is_null_on;
	}
	bindi[9].length= 0;

	bindi[10].buffer_type= MYSQL_TYPE_STRING;
	bindi[10].buffer= name2;
	if (name2) {
		name2l = strlen(name2);
		bindi[10].buffer_length= name2l;
		bindi[10].is_null= 0;
		bindi[10].length= &name2l;
	} else {
		bindi[10].is_null= &is_null_on;
	}

	bindi[11].buffer_type= MYSQL_TYPE_TIME;
	bindi[11].buffer= (char *)ts2;
	if (ts2) {
		bindi[11].is_null= 0;
	} else {
		bindi[11].is_null= &is_null_on;
	}	
	bindi[11].length= 0;
	
	bindi[12].buffer_type= MYSQL_TYPE_LONG;
	bindi[12].buffer= (char *)i2;
	if (i2) {
		bindi[12].is_null= 0;
	} else {
		bindi[12].is_null= &is_null_on;
	}
	bindi[12].length= 0;
	...
	memset(&ts_res1, 0, sizeof(ts_res1));
	memset(&ts_res3, 0, sizeof(ts_res3));
	memset(&ts_res4, 0, sizeof(ts_res4));
	...
	if (rc) {
		...
		diag("name3 expected/retrieved: %s , %s", name1 ? name1 : "NULL" , is_null[7] ? "NULL" : (char *)binds[7].buffer);
		if (name1 == NULL && is_null[7]) {
			matches++;
		} else {
			if (name1 && !is_null[1] && strcmp(name1,(char *)binds[7].buffer)==0)
			matches++;
		}
		if (ts1) {
			sprintf(buf1,"%d:%d:%d", ts1->hour, ts1->minute, ts1->second);
		...
		if (is_null[8]) {
			...
			sprintf(buf2,"%d:%d:%d", ts_res3.hour, ts_res3.minute, ts_res3.second);
		}
		if (strcmp(buf1,buf2)==0)
			matches++;
		diag("time3 expected/retrieved: %s , %s", buf1, buf2);
		sprintf(buf1,"NULL");
		sprintf(buf2,"NULL");
		if (i1)
			sprintf(buf1, "%d", *i1);
		if (!is_null[9])
			sprintf(buf2, "%d", i3_res);
		diag("i3 expected/retrieved: %s , %s", buf1, buf2);
		if (strcmp(buf1,buf2)==0)
			matches++;
		diag("name4 expected/retrieved: %s , %s", name2 ? name2 : "NULL" , is_null[10] ? "NULL" : (char *)binds[10].buffer);
		if (name2 == NULL && is_null[10]) {
			matches++;
		} else {
			if (name2 && !is_null[10] && strcmp(name2,(char *)binds[10].buffer)==0)
			matches++;
		}
		if (ts2) {
			sprintf(buf1,"%d:%d:%d", ts2->hour, ts2->minute, ts2->second);
		...
		if (is_null[11]) {
			...
			sprintf(buf2,"%d:%d:%d", ts_res4.hour, ts_res4.minute, ts_res4.second);
		}
		if (strcmp(buf1,buf2)==0)
			matches++;
		diag("time4 expected/retrieved: %s , %s", buf1, buf2);
		sprintf(buf1,"NULL");
		sprintf(buf2,"NULL");
		if (i2)
			sprintf(buf1, "%d", *i2);
		if (!is_null[12])
			sprintf(buf2, "%d", i4_res);
		diag("i4 expected/retrieved: %s , %s", buf1, buf2);
		if (strcmp(buf1,buf2)==0)
			matches++;
		ok(matches == 13, "Matching columns for ID %d/%d: %d (expected 13)\n", id, copyid, matches);
...
int main(int argc, char** argv) {
	...
	MYSQL_QUERY(proxysql_mysql, "DROP TABLE IF EXISTS test.reg_test_3585_13");
	MYSQL_QUERY(proxysql_mysql, "CREATE TABLE IF NOT EXISTS test.reg_test_3585_13 (`id` int(11) NOT NULL, `name1` varchar(255) DEFAULT NULL, `time1` time DEFAULT NULL, i1 INT DEFAULT NULL, `name2` varchar(255) DEFAULT NULL, `time2` time DEFAULT NULL, i2 INT DEFAULT NULL, `name3` varchar(255) DEFAULT NULL, `time3` time DEFAULT NULL, i3 INT DEFAULT NULL, `name4` varchar(255) DEFAULT NULL, `time4` time DEFAULT NULL, i4 INT DEFAULT NULL, PRIMARY KEY (id)) ENGINE=InnoDB");


	int rc;
	std::string query_i = "INSERT INTO test.reg_test_3585_13 VALUES (? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)";
	// Force the 'hostgroup' for the 'SELECT' query to avoid replication issues
	std::string query_s = "SELECT /*+ ;hostgroup=0 */ * FROM test.reg_test_3585_13 WHERE id=?";
