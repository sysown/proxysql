/**
 * @file reg_test_xxxx-USE_with_comment-t.cpp
 ...
 *   2. Drops and creates a new database called 'reg_test_xxxx_use_comment'.
...
int main(int argc, char** argv) {
	...
	MYSQL_QUERY(proxysql_mysql, "DROP DATABASE IF EXISTS reg_test_xxxx_use_comment");
	MYSQL_QUERY(proxysql_mysql, "CREATE DATABASE reg_test_xxxx_use_comment");

	int err = mysql_query(proxysql_mysql, "/*+ placeholder_comment */ USE reg_test_xxxx_use_comment");
	if (err) {
		diag(
	...
	ok(
		database_name == "reg_test_xxxx_use_comment",
		"Selected DB name should be equal to actual DB name: (Exp: '%s') == (Act: '%s')",
		"reg_test_xxxx_use_comment",
		database_name.c_str()
	);
