 ...
 */
bool trigger_auto_increment_delay_multiplex_metric(MYSQL* proxy, MYSQL*) {
	int inc_query_res =
		mysql_query(
			proxy,
...
bool trigger_access_denied_wrong_password_total(MYSQL*, MYSQL*) {
	...
	if(!access_denied_error)
		diag("Connections should have failed due to access denied. ErrCode: %d", 0);
...
bool trigger_transaction_rollback_total(MYSQL* proxysql, MYSQL*) {
...
bool get_proxysql_version_info(MYSQL*, MYSQL* admin) {
...
bool rm_add_server_connpool_setup(MYSQL* proxy, MYSQL* admin) {
...
bool rm_add_server_connpool_counters(MYSQL* proxy, MYSQL* admin) {
...
using setup = function<bool(MYSQL*, MYSQL*)>;
using metric_trigger = function<bool(MYSQL*, MYSQL*)>;
...
bool placeholder_setup(MYSQL*, MYSQL*) { return true; }
...
int main(int argc, char** argv) {
	...
	plan(
		4*metric_tests.size() + // connections
		2*2 + // connections
		metric_tests.size() * 4 // tests
		);
	...
	for (const auto& metric_test : metric_tests) {
		...
		bool action_res = metric_setup(proxysql, proxysql_admin);
		...
		bool trigger_res = metric_trigger(proxysql, proxysql_admin);
