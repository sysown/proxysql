...
void *mysql_thread(int tid) {
  ...
  if (!mysql_real_connect(mysql, "127.0.0.1", "admin", "admin", "main", 6032,
                          NULL, 0)) {
  ...
  while (__sync_fetch_and_add(&shutdown_test, 0) == 0) {
    if ((uint32_t)mt_rand() % 10) { // reconnect
      ...
      if (!mysql_real_connect(mysql, "127.0.0.1", "msandbox", "msandbox", NULL,
                              6033, NULL, 0)) {
        fprintf(stderr, "Failed to connect: Error: %s\n", mysql_error(mysql));
        exit(EXIT_FAILURE);
      }
...
void *setup_admin() {
  ...
  while (__sync_fetch_and_add(&shutdown_test, 0) == 0) {
    ...
    hashed[0] = hashed[1];
    if ((uint32_t)mt_rand() % 2) {
      query = (char *)"UPDATE mysql_users SET password='msandbox' WHERE "
                      "username='msandbox'";
      hashed[1] = false;
    } else {
      query = (char *)"UPDATE mysql_users SET "
                      "password='*6c387fc3893dba1e3ba155e74754da6682d04747' "
                      "WHERE username='msandbox'";
      hashed[1] = true;
    }
