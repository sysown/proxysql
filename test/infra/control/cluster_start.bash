    ...
    for i in $(seq 1 "${NUM_NODES}"); do
        ...
        if [ ! "$(docker ps -q -f name=${CONTAINER_NAME})" ]; then
            ...
            if ! docker run -d \
                ...
                -v "${CLUSTER_CONFIG}:/etc/proxysql.cnf" \
                -v "${NODE_DIR}:/var/lib/proxysql" \
                proxysql-ci-base:latest \
                /bin/bash -c "/usr/bin/proxysql --idle-threads --clickhouse-server --sqlite3-server -f -c /etc/proxysql.cnf -D /var/lib/proxysql 2>&1 | tee /var/lib/proxysql/proxysql.log"; then
...
for i in $(seq 1 "${NUM_NODES}"); do
    ...
    while [ $COUNT -lt $MAX_WAIT ]; do
        if command -v docker &> /dev/null; then
            ...
            if mysql -uradmin -pradmin -h "${NODE_NAME}" -P 6032 -e 'SELECT 1' >/dev/null 2>&1; then
                echo " OK."
                break
            fi
        fi
