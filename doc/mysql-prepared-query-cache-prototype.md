...
existing user identity + binary-cache domain + destination hostgroup
  + hash(exact prepared-SQL identity, execute bytes from flags through end,
         effective raw parameter-type block)
```
...
appropriate reads: session-dependent expressions, SQL modes, temporary tables,
schema changes and write freshness still need care. The extra transaction
...
constraint bypass, hard expiry and transaction bypass. Repeat with
