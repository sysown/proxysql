...
// MariaDB client library redefines dlerror(), see
// https://mariadb.atlassian.net/browse/CONC-101
#ifdef dlerror
#undef dlerror
#endif
...
void ProxySQL_Main_init_SSL_module() {
        ...
        if ( SSL_CTX_use_certificate_file(GloVars.global.ssl_ctx, "newreq.pem",
SSL_FILETYPE_PEM) <= 0 )	{ ERR_print_errors_fp(stderr); abort();
        }
        if ( SSL_CTX_use_PrivateKey_file(GloVars.global.ssl_ctx, "privkey.pem",
SSL_FILETYPE_PEM) <= 0 ) { ERR_print_errors_fp(stderr); abort();
        }
...
// few examples tests to demonstrate the ability to add and remove listeners at
runtime GloMTH->listener_add((char *)"0.0.0.0:6033"); sleep(3);
...
// "xmalloc:true,lg_tcache_max:16,purge:decay,junk:true,tcache:false"; #else
const char *malloc_conf = "xmalloc:true,lg_tcache_max:16,purge:decay";
//#endif /* DEBUG */
// const char *malloc_conf =
// "prof_leak:true,lg_prof_sample:0,prof_final:true,xmalloc:true,lg_tcache_max:16";
...
void ProxySQL_Main_process_global_variables(int argc, const char **argv) {
  ...
  if (GloVars.__cmd_proxysql_initial == true) {
    ...
    rename(GloVars.admindb, newpath); // FIXME: should we check return value, or
                                      // ignore whatever it successed or not?
  }
...
void ProxySQL_Main_init_Query_Cache_module() {
  ...
  GloQC->print_version();
  pthread_create(&GloQC->purge_thread_id, NULL, mysql_shared_query_cache_funct,
                 NULL);
}
...
void ProxySQL_Main_init_phase4___shutdown() {
  ...
  // write(GloAdmin->pipefd[1], &GloAdmin->pipefd[1], 1);	// write a
  // random byte
  if (GloVars.global.nostart) {
...
int main(int argc, const char *argv[]) {
  ...
  if (GloVars.global.foreground == false) {
    ...
    gotofork:
      if (laststart) {
        // daemon_log(LOG_INFO, "Angel process is waiting %d seconds before
        // starting a new ProxySQL process\n", glovars.proxy_restart_delay);
        proxy_info("Angel process is waiting %d seconds before starting a new "
                   "ProxySQL process\n",
                   glovars.proxy_restart_delay);
