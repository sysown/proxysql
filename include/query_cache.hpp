...
struct __QC_entry_t {
  ...
  unsigned long long expire_ms; // when the entry will expire, monotonic ,
                                // millisecond granularity
  unsigned long long access_ms; // when the entry was read last , monotonic ,
                                // millisecond granularity
...
public:
  ...
  ~Query_Cache();
  void print_version();
  bool set(uint64_t, const unsigned char *, uint32_t, unsigned char *, uint32_t,
