...
private:
  void shrink() {
    ...
    myds = (MySQL_Data_Stream **)realloc(myds, new_size *
                                                   sizeof(MySQL_Data_Stream *));
...
public:
  ...
  ~ProxySQL_Poll() {
    unsigned int i;
    for (i = 0; i < len; i++) {
      if (myds[i] && // fix bug #278 . This should be caused by not initialized
                     // datastreams used to ping the backend
...
public:
  ...
  // in this way, there is no need for atomic operation and there is no cache
  // miss when it is needed a total, all threads are checked
  struct {
  ...
  // void myds_backend_set_failed_connect(MySQL_Data_Stream *myds, unsigned int
  // n);  void myds_backend_pause_connect(MySQL_Data_Stream *myds);  void
  // myds_backend_first_packet_after_connect(MySQL_Data_Stream *myds, unsigned
  // int n);
