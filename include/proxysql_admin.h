...
// void fill_table__server_status(SQLite3DB *db);
  ...
  void flush_mysql_variables___runtime_to_database(SQLite3DB *db, bool replace,
                                                   bool del, bool onlyifempty,
                                                   bool runtime = false);
  ...
  void flush_admin_variables___runtime_to_database(SQLite3DB *db, bool replace,
                                                   bool del, bool onlyifempty,
                                                   bool runtime = false);
...
public:
  ...
  //	void SQLite3_to_MySQL(SQLite3_result *result, char *error, int
  //affected_rows, MySQL_Protocol *myprot);
...
//	virtual void admin_session_handler(MySQL_Session *sess);
  ...
  void GenericRefreshStatistics(const char *query_no_space,
                                unsigned int query_no_space_length, bool admin);
