...
/*
        Enabling STRESSTEST_POOL ProxySQL will do a lot of loops in the
connection pool This is for internal testing ONLY!!!! #define STRESSTEST_POOL
*/
...
#define MYHGM_MYSQL_SERVERS                                                    \
  ...
  "CHECK (weight >= 0) NOT NULL DEFAULT 1 , status INT CHECK (status IN (0, "  \
  "1, 2, 3, 4)) NOT NULL DEFAULT 0 , compression INT CHECK (compression >=0 "  \
  "AND compression <= 102400) NOT NULL DEFAULT 0 , max_connections INT CHECK " \
  ...
  ", comment VARCHAR NOT NULL DEFAULT '' , mem_pointer INT NOT NULL DEFAULT "  \
  "0 , PRIMARY KEY (hostgroup_id, hostname, port) )"
#define MYHGM_MYSQL_SERVERS_INCOMING                                           \
  "CREATE TABLE mysql_servers_incoming ( hostgroup_id INT NOT NULL DEFAULT 0 " \
  ", hostname VARCHAR NOT NULL , port INT NOT NULL DEFAULT 3306 , weight INT " \
  "CHECK (weight >= 0) NOT NULL DEFAULT 1 , status INT CHECK (status IN (0, "  \
  "1, 2, 3, 4)) NOT NULL DEFAULT 0 , compression INT CHECK (compression >=0 "  \
  "AND compression <= 102400) NOT NULL DEFAULT 0 , max_connections INT CHECK " \
...
public:
  ...
  // note that these variables are in microsecond, while user defines max
  // lantency in millisecond
  ...
  bool shunned_and_kill_all_connections; // if a serious failure is detected,
                                         // this will cause all connections to
                                         // die even if the server is just
                                         // shunned
  bool use_ssl;
  char *comment;
  // uint8_t charset;
