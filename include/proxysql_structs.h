...
enum debug_module {
  ...
  PROXY_DEBUG_UNKNOWN // this module doesn't exist. It is used only to define
                      // the last possible module
};
...
struct _global_variable_entry_t {
  ...
  void (*func_post)(
      global_variable_entry_t *); // function called after initializing variable
};
...
/* Every communication between client and proxysql, and between proxysql and
 * mysql server is performed within a mysql_data_stream_t
 */
struct _mysql_data_stream_t {
  mysql_session_t *sess; // pointer to the session using this data stream
  // MySQL_Backend *mybe;	// if this is a connection to a mysql server, this
  // points to a backend structure
  ...
  int active;             // data stream is active. If not, shutdown+close needs to be
                          // called
...
{
//	{ "mysql-port", 0, 0, G_OPTION_ARG_INT, &__cmd_proxysql_mysql_port,
"MySQL proxy port", NULL },
//	{ "admin-socket", 'S', 0, G_OPTION_ARG_FILENAME,
&__cmd_proxysql_admin_socket, "Administration Unix Socket", NULL },
//	{ "admin-socket", 'S', 0, G_OPTION_ARG_FILENAME,
&GloVars.__cmd_proxysql_admin_socket, "Administration Unix Socket", NULL },
//	{ "no-start", 'n', 0, G_OPTION_ARG_NONE,
&GloVars.__cmd_proxysql_nostart, "Starts only the admin service", NULL },
//	{ "foreground", 'f', 0, G_OPTION_ARG_NONE,
&GloVars.__cmd_proxysql_foreground, "Run in foreground", NULL },
//	{ "version", 'V', 0, G_OPTION_ARG_NONE,
&GloVars.__cmd_proxysql_print_version, "Print version", NULL },
//#ifdef DEBUG
//	{ "debug", 'd', 0, G_OPTION_ARG_NONE, &GloVars.__cmd_proxysql_gdbg,
"debug", NULL },
//#endif // DEBUG
//	{ "datadir", 'D', 0, G_OPTION_ARG_FILENAME,
&GloVars.__cmd_proxysql_datadir, "Datadir", NULL },
//	{ "admin-pathdb", 'a', 0, G_OPTION_ARG_FILENAME,
&GloVars.__cmd_proxysql_admin_pathdb, "Configuration DB path", NULL },
//	{ "config", 'c', 0, G_OPTION_ARG_FILENAME,
&GloVars.__cmd_proxysql_config_file, "Configuration text file", NULL }, { NULL }
};
*/
