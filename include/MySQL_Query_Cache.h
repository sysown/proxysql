#ifndef PROXYSQL_MYSQL_QUERY_CACHE_H
#define PROXYSQL_MYSQL_QUERY_CACHE_H
...
public:
	...
	bool set(uint64_t user_hash, const unsigned char* kp, uint32_t kl, unsigned char* vp, uint32_t vl, 
		uint64_t create_ms, uint64_t curtime_ms, uint64_t expire_ms, bool deprecate_eof_active, uint64_t num_rows = 0);
	unsigned char* get(uint64_t user_hash, const unsigned char* kp, const uint32_t kl, uint32_t* lv, 
		uint64_t curtime_ms, uint64_t cache_ttl, bool deprecate_eof_active, uint64_t *num_rows = nullptr);
