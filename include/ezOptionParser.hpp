...
had to start with alpha). v0.1.3 20120108 rsz Created work-around for unique id
generation with IDGenerator that avoids retarded c++ translation unit linker
errors with single-header static variables. Forced inline on all methods to
please retard compiler and avoid multiple def errors. v0.1.4 20120629 Enforced
MIT license on all files. v0.2.0 20121120 Added parseIndex to OptionGroup.
...
requiring a test such as <x<, <=x<, <x<=, <=x<=. A regcomp/regexec based class
...
public:
  ...
  // arguments. It's the second section to get printed in the full usage
  // message.
  std::string syntax;
...
void ezOptionParser::getUsageDescriptions(std::string &usage, int width,
  ...
  // or gap between flag names and description (align, stagger layouts).
  int gutter = 3;
