...
public:
  ...
  bool insert(QP_rule_t *qr, bool lock = true); // insert a new rule. Uses a
                                                // generic void pointer to a
                                                // structure that may vary
                                                // depending from the Query
                                                // Processor
                                                //	virtual bool insert_locked(QP_rule_t *qr) {return false;};		//
  //call this instead of insert() in case lock was already acquired via wrlock()
  QP_rule_t *new_query_rule(
      ...
      int retries, int delay, int mirror_hostgroup, int mirror_flagOUT,
      char *error_msg, int sticky_conn, int multiplex, int log, bool apply,
      char *comment); // to use a generic query rule struct, this is generated
                      // by this function and returned as generic void pointer
  void delete_query_rule(QP_rule_t *qr); // destructor
  // virtual bool remove(int rule_id, bool lock=true) {return false;}; // FIXME:
  // not implemented yet, should be implemented at all ?
  //	virtual bool remove_locked(int rule_id) {return false;};		// call
  //this instead of remove() in case lock was already acquired via wrlock()
  ...
  void update_query_digest(SQP_par_t *qp, int hid,
                           MySQL_Connection_userinfo *ui, unsigned long long t,
                           unsigned long long n,
                           MySQL_STMT_Global_info *_stmt_info);

  unsigned long long
  query_parser_update_counters(MySQL_Session *sess,
                               enum MYSQL_COM_QUERY_command c, SQP_par_t *qp,
