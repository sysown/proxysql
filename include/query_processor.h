...
class Query_Processor {
	...
	Query_Processor_Output * process_mysql_query(Client_Session *sess, void *ptr, unsigned int size, Query_Info *qi);
	...
	void update_query_digest(SQP_par_t *qp, int hid, MySQL_Connection_userinfo *ui, unsigned long long t, unsigned long long n, MySQL_STMT_Global_info *_stmt_info, Client_Session *sess);

	unsigned long long query_parser_update_counters(Client_Session *sess, enum MYSQL_COM_QUERY_command c, SQP_par_t *qp, unsigned long long t);
