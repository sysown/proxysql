...
The source archive is vendored directly in git (like every other
dependency under `deps/`). No Git LFS, no extra fetch step — a plain
`git clone` contains the full archive. Verify it before building:

```bash
deps/libssl/verify-source.bash
```

The verifier rejects missing files, checksum mismatches, invalid gzip
data, unsafe archive paths, and unexpected archive layouts before
extraction.
